...
% Authors: Fred J. Hickernell,Gen Li, Hanyu Hu
...
% properties particulary for this class
   properties (SetAccess=public) %so they can only be set by the constructor
      assetParam = struct('pathType', 'GBM', ... %type of asset path
         ...
         'interest', [0.00,0.01,0.00], ... %interest rate                               
   ...
   methods
      ...
      % Generate square root of correlation matrix
       ...
       function [paths]=genPaths(obj,val)
         bmpaths_r = genPaths@brownianMotion(obj,val);
         bmpaths = genPaths@brownianMotion(obj,val);
         nPaths = size(bmpaths,1);
         r_paths = zeros(nPaths,obj.timeDim.nSteps);
         r_paths(:,1) = obj.assetParam.interest(2);
         for i=2:obj.timeDim.nSteps 
                  dr = obj.assetParam.interest(1)*(obj.assetParam.interest(2)-r_paths(:,i-1))...
                        *obj.timeDim.timeIncrement(i-1)+obj.assetParam.interest(3)...
                            *bmpaths_r(:,i-1);
                  r_paths(:,i) = r_paths(:,i-1)+dr;
         end
         obj.assetParam.interest = mean(mean(r_paths));  
         r_paths = repmat(r_paths,1,obj.assetParam.nAsset);
         ...
         if strcmp(obj.assetParam.pathType,'GBM')
            ...
            for idx=1:obj.assetParam.nAsset
              ...
              paths(:,colRange) = obj.assetParam.initPrice(idx) * ...
                 exp(bsxfun(@plus,(r_paths(:,colRange) ...
                 - obj.assetParam.volatility(idx).^2/2) ...
                 .* repmat(obj.timeDim.timeVector,nPaths,1), obj.assetParam.volatility(idx)...
                 .* tempc));
             end
         end
      end
