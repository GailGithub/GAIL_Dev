function [tmu,out_param]=meanMC_CLT_AIS_regression(fx,abstol,alpha,nSig,fudge)
...
b=[1,2,3,4];%estimated values for standard deviation.
...
%Regression to determine the best value of b
xr=b;
yr=var_b;
l=numel(xr);
%linear
r2_linear=((((l.*sum(xr.*yr))-sum(xr).*sum(yr))./(sqrt((l.*sum(xr.^2))-(sum(xr).^2)).*(sqrt(l.*sum(yr.^2))-(sum(yr).^2)))).^2)
a1=((l.*sum(xr.*yr))-sum(xr).*sum(yr))./((l.*sum(xr.^2))-(sum(xr).^2));
a0=(sum(yr)./l)-((a1.*sum(xr)./l));

%power
xl=log(xr);
yl=log(yr);;
r=(((l.*sum(xl.*yl))-sum(xl).*sum(yl))./(sqrt((l.*sum(xl.^2))-(sum(xl).^2)).*(sqrt(l.*sum(yl.^2))-(sum(yl).^2))));
r2_power=r.^2

%exponential
r=(((l.*sum(xr.*yl))-sum(xl).*sum(yl))./(sqrt((l.*sum(xr.^2))-(sum(xr).^2)).*(sqrt(l.*sum(yl.^2))-(sum(yl).^2))));
r2_exponential=r.^2
