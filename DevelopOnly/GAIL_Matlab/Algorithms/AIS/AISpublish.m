%% Summer Research 2015 - Monte Carlo Methods
%
...
% vectors. 
%
% If \(\boldsymbol{X}\) is a random variable with probability density
% function $\rho$, then $\mu$ can be represented as the mean of a function
% of \(\boldsymbol{X}\).
%
% \begin{equation*} \mu =
% \int_{\mathbb{R}^d}\frac{f(\boldsymbol{x})}{\rho(\boldsymbol{x})}
% \rho(\boldsymbol{x}) \, {\rm d} \boldsymbol{x} \end{equation*}
%
% Importance Sampling proceeds by choosing an importance function \(g\)
% defined as:
%
% \begin{equation*} g(\boldsymbol{x}) := \frac{f(\boldsymbol{x})}
% {\rho(\boldsymbol{x})}, \qquad \boldsymbol{x} \in \mathbb{R}^d \end{equation*}
%
% Applying \(g\) in the integral:  
%
% \begin{equation*} \int_{\mathbb{R}^d}g(\boldsymbol{x}) \rho(\boldsymbol{x}) \, {\rm d}
...
% where it matters.
%
% Adaptive Importance Sampling (AIS) is a mechanical but flexible way of
% dealing with the selection of parameters of the importance function. For
% AIS to be accurate the variance of the mean, \(V_p(\hat{\mu}_n)\), must be small. 
...
%% Example 1:
%
% Consider the following integral and Gaussian probability
% density $\rho$:
% 
% \begin{equation*} 
...
%
% Where \begin{equation*} g(\boldsymbol{x}) = (\sqrt{2\pi}b)^d \cos(b\lVert
% \boldsymbol{z} \rVert) \exp((1/2-b^2)\lVert \boldsymbol{z} \rVert^2)
% \end{equation*} and \begin{equation*} \rho(\boldsymbol{x}) = \frac{\exp(-\lVert \boldsymbol{z} \rVert^2)/2}{(\sqrt{2\pi})^d}
% \end{equation*}
%
% Using \(g\) as an input, our algorithm will determine the value
% of *b*, within a determined interval, for which the variance is the
% smallest.

b = [0.05 1]; abstol = 0.002; alpha = 0.01; nSig = 1e4; fudge = 1.2; d=1;
Y1=@(z,b) ((sqrt(2.*pi).*b).^d).*cos(b.*sqrt(sum(z.*z,2))).*exp((1/2-b.^2).*sum(z.*z,2));
[tmu,out_param]=meanMC_CLT_AIS(Y1,b,d,abstol,alpha,nSig,fudge)
...
%% Example 2: 
% Using the same parameters of the Example 1:
...
% * OH, M. BERGER, J. O. _*Adaptive Importance Sampling In Monte Carlo Integration*_.  
% Purdue University October 1989
%
%% Authors
% 
