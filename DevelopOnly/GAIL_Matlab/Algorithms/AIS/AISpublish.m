...
% *Example 1:*
d=3;
[tmu,out_param]=meanMC_CLT_AIS(@(z,b) ((sqrt(2.*pi).*b).^d).*cos(b.*sqrt(sum(z.*z,2)))...
    .*exp((1/2-b.^2).*sum(z.*z,2)))
...
[tmu,out_param]=meanMC_AIS_g(@(z,b) ((sqrt(2.*pi).*b).^d).*cos(b.*sqrt(sum(z.*z,2)))...
    .*exp((1/2-b.^2).*sum(z.*z,2)),[0.5 2])
