...
T = 2;      
K = S0;
Yrand = @(n) max(S0*exp(((sigma*sigma)/2)*T+sigma*sqrt(T/d)*sum(randn(n,d),2)) ,0) - K;    %new Yrand function (CPU)
Yrand_GPU = @(n) max(S0*exp(((sigma*sigma)/2)*T+sigma*sqrt(T/d)*sum(gpuArray.randn(n,d),2)) ,0) - K;   %new Yrand function (GPU)
for i = X
    %run on CPU
    t = tic;
...
Y = [1,2,3];                         %this vector will store the runtimes (CPU)
Y_GPU = [1,2,3];                     %this vector will store the runtimes (GPU)
Yrand = @(n) sin(rand(n,1));         %new Yrand function (CPU)
Yrand_GPU = @(n) sin(gpuArray.rand(n,1));     %new Yrand function (GPU)
for i = X
    %run on CPU
    t = tic;
    meanMC_CLT(Yrand,1e-3);          %function and error tolerance as parameters
...
Y = [1,2,3];                         %this vector will store the runtimes (CPU)
Y_GPU = [1,2,3];                     %this vector will store the runtimes (GPU)
Yrand = @(n) exp(sin(rand(n,1)));        %new Yrand function (CPU)
Yrand_GPU = @(n) exp(sin(rand(n,1)));     %new Yrand function (GPU)
for i = X
    %run on CPU
    t = tic;
    meanMC_CLT(Yrand,1e-3);          %function and error tolerance as parameters
