...
if isempty(varargin) % if no value was parsed
    ...
    warning('GAIL:meanMCBer_g:yrandnotgiven',...
...
if (~gail.isfcn(Yrand))
    warning('GAIL:meanMCBer_g:yrandnotfcn',...
...
if max(size(Yrand(5)))~=5 || min(size(Yrand(5)))~=1
    ...
    warning('GAIL:meanMCBer_g:yrandnotlengthN',...
...
if (out_param.abstol < 0) %absolute error tolerance negative
    warning('GAIL:meanMCBer_g:abstolneg',...
...
if (out_param.alpha <= 0 ||out_param.alpha >= 1) 
    ...
    warning('GAIL:meanMCBer_g:alphanotin01',...
...
if (~gail.isposge30(out_param.nmax)) 
    ...
    warning('GAIL:meanMCBer_g:nmaxnotposint',...
...
switch out_param.exit
    case 1 % nabs exceed nmax.
        warning('GAIL:meanMCBer_g:nabsexceednmax',...
