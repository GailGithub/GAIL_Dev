...
if isempty(varargin)
    help meanMCdep_g
    warning('GAIL:meanMCdep_g:yrandnotgiven',...
...
if (~gail.isfcn(Yrand))
    warning('GAIL:meanMCdep_g:yrandnotfcn',...
...
if max(size(Yrand(5)))~=5 || min(size(Yrand(5)))~=1
    ...
    warning('GAIL:meanMCdep_g:yrandnotlengthN',...
...
if (out_param.abstol < 0)
    ...
    warning('GAIL:meanMCdep_g:abstolneg',...
...
if (out_param.reltol < 0 || out_param.reltol > 1)
    ...
    warning('GAIL:meanMCdep_g:reltolneg',...
...
if (out_param.alpha <= 0 ||out_param.alpha >= 1) 
    ...
    warning('GAIL:meanMCdep_g:alphanotin01',...
...
if (out_param.fudge <= 1) 
    ...
    warning('GAIL:meanMCdep_g:fudgelessthan1',...
...
if (~gail.isposge30(out_param.nSig)) 
    ...
    warning('GAIL:meanMCdep_g:nsignotposint',...
...
if (~gail.isposge30(out_param.n1)) 
    ...
    warning('GAIL:meanMCdep_g:n1notposint',...
...
if (out_param.tbudget < 0) 
    ...
    warning('GAIL:meanMCdep_g:timebudgetneg',...
        ['Time budget in seconds should be positive; '...
...
if (out_param.tbudget == 0) 
    %time budget in seconds should be positive
    warning('GAIL:meanMCdep_g:timebudget0',...
...
if (~gail.isposge30(out_param.nbudget)) 
    ...
    warning('GAIL:meanMCdep_g:nbudgetnotposint',...
...
switch out_param.exit
    case 1 % not enough samples to estimate the mean.
        ...
        warning('GAIL:meanMCdep_g:maxreached',...
