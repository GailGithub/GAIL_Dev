...
% approximation
classdef cubMLELatticeClass < handle
    ...
    properties (SetAccess = private)
        ff = NULL; %integrand after the periodization transform
    ...
    methods
        ...
        function obj = cubMLELatticeClass(varargin)  %Constructor
    
            if nargin > 0
                iStart = 1;
                if isa(varargin{1},'cubMLELatticeClass')
                    obj = copy(varargin{1});
                    iStart = 2;
                end
                if nargin >= iStart
            ...
            obj.ff = cubMLELatticeClass.doPeriodTx(obj.f, obj.ptransform);
        end
        ...
        function [muhat,out] = compInteg(obj)
            ...
            for iter = 1:numM
                ...
                if iter == 1
                    ...
                    xun = cubMLELatticeClass.simple_lattice_gen(n,d,true);
                    ...
                    xunnew = cubMLELatticeClass.simple_lattice_gen(n,d,false);
        ...
        function [loss,Ktilde,Kthat_new,RKHSnorm,K] = MLEKernel(obj, a,xun,ftilde)
            ...
            n = length(ftilde);
            if obj.order==4
                [K, Ktilde, Kthat_new] = cubMLELatticeClass.kernel(xun,obj.order,a);
            elseif obj.order==2
                [K, Ktilde, Kthat_new] = cubMLELatticeClass.kernel(xun,obj.order,a);
            else
    ...
    methods(Static)  
        ...
        function [K, Ktilde, Kthat_new] = kernel(xun,order,a)
            ...
            [Kt_new, K_new] = cubMLELatticeClass.kernel_t(a, constMult, bernPloy(xun));
        ...
        function xlat = simple_lattice_gen(n,d,firstBatch)
            ...
            if false
                ...
                if firstBatch==true
                    brIndices=cubMLELatticeClass.vdc(nelem)';
                else
                    brIndices=cubMLELatticeClass.vdc(nelem)'+1/(2*(nmin-1));
                end
            end
