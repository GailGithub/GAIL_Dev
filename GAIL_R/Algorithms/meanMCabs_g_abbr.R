...
meanMCabs_g_abbr = function (Yrand = function(n) {runif(n)^2},abstol=0.01,alpha=0.01,nSig=100,fudge=1.2){
  ...
  cat("Number of samples required = ", nmu, ". Mean will be estimated using 1e8 samples\n")
  tmu=mean(Yrand(1e8)); #estimated mean
  out_param.ntot=nSig+1e8; #total samples required
