...
meanMCabs_g_abbr = function (Yrand = function(n) {runif(n)^2},abstol=0.01,alpha=0.01,nSig=100,fudge=1.2){
  ...
  if(nmu>=nMax) {message("Program stopped due to the guarantee needing over", nMax, " samples")
  cat("Number of samples required = ", nmu, ". Mean will be estimated using ", nMax, " samples\n")
  tmu=mean(Yrand(nMax)); #estimated mean
  out_param.ntot=nSig+nMax; #total samples required
