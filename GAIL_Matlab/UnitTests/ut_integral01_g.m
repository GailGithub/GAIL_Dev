...
classdef ut_integral01_g < matlab.unittest.TestCase    
    methods (Test)
        ...
        function testerrorfabstol(testCase)
            f=@(x) 3*x.^2;
            abstol=1e-7;
            actSolution = integral01_g(f,'abstol',abstol);
            expSolution = 1;
        ...
        function testerrorfninit(testCase)
            ...
            f=@(x) 3*x.^2;
            ninit=52;
            actSolution = integral01_g(f,'ninit',ninit);
            expSolution = 1;
            import matlab.unittest.constraints.IsLessThanOrEqualTo
            testCase.verifyThat(abs(actSolution-expSolution),...
                IsLessThanOrEqualTo(abstol));
        ...
        function testerrorfnamx(testCase)
            ...
            abstol=1e-7;
            nmax=1e6;
            actSolution = integral01_g(f,'nmax',nmax);
            expSolution = 1;
            import matlab.unittest.constraints.IsLessThanOrEqualTo
            testCase.verifyThat(abs(actSolution-expSolution),...
                IsLessThanOrEqualTo(abstol));
        ...
        function testerrorfabstolninit(testCase)
            ...
            abstol=1e-7;
            ninit=52;
            actSolution = integral01_g(f,'abstol',abstol,'ninit',ninit);
            expSolution = 1;
            import matlab.unittest.constraints.IsLessThanOrEqualTo
            testCase.verifyThat(abs(actSolution-expSolution),...
                IsLessThanOrEqualTo(abstol));
        ...
        function testerrorfabstolnmax(testCase)
            ...
            abstol=1e-7;
            nmax=1e6;
            actSolution = integral01_g(f,'abstol',abstol,'nmax',nmax);
            expSolution = 1;
            import matlab.unittest.constraints.IsLessThanOrEqualTo
            testCase.verifyThat(abs(actSolution-expSolution),...
                IsLessThanOrEqualTo(abstol));
