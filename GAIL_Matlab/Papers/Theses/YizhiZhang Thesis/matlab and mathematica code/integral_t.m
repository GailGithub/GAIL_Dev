...
Varfpup=[inf; zeros(10,1)]; %initialize vector of upper bounds to Var(f') (eta)
ii=1; %index

if intervallen>0
    steplen=intervallen/ntrap; % length of subinterval (h)
    steplenvec=zeros(10,1); %vector to record subinterval lengths
    hcut=out_param.hcut; %maximum interval size (cut-off)
    inflatelim=1.5; %(C(0))
    ...
    while true
        ...
        if Varfp(ii) > Varfpup(ii+1) %f lies outside cone
            %Decrease hcut
            tempa=1-inflatelim*hcut/(hcut-2*steplen)*Varfp(1:ii)/Varfp(ii); %to check j=1:k upper and kth lower
        ...
        if errest <= out_param.abstol %tolerance is satisfied
            ...
            inflation=max(2,ceil(steplen*sqrt(Varfp(ii)/(8*out_param.abstol))));
        end
...
out_param.ninit = max(ceil(2*(out_param.b-out_param.a)/out_param.hcut)+2,3);
