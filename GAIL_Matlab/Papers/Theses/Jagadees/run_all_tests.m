...
for arbMean=arbMeanType
  if arbMean==true
    figSavePath = strcat(figSavePath, 'arbMean/');
  else
    figSavePath = strcat(figSavePath, 'zeroMean/');
  end
  for tx=pdTx
    for dim=[2 3 4]
      for bern=[4 2]
        TestExpCosBayesianCubature(dim,bern,tx{1},figSavePath,visiblePlot,arbMean)
        TestKeisterBayesianCubature(dim,bern,tx{1},figSavePath,visiblePlot,arbMean)
        if dim~=4
          TestMVN_BayesianCubature(dim,bern,tx{1},figSavePath,visiblePlot,arbMean)
        end
      end
    end
  end
