...
for k = 1:kMax-1
    for d = 1:dMax
        ...
        for j = 1:nFunc
            ...
            WorError(k,d) = max(WorError(k,d),sqrt(norm(Pf-exact)));
        end
        WorError(k,d) = WorError(k,d)/SqrtNTest;
    end
end
%% plot
fontSize = 16;
loglog(N,WorError);
hold on;
plot(N,16*N.^(-sqrt(rAlpha*rGamma)),'m--o');
xlabel('Number of Data Points','FontSize',fontSize);
ylabel('RMSE','FontSize',fontSize);
%title('Interpolation With General Gaussian (Worst Case)','FontSize',14);
