...
for i = 1:nrep
  ...
  f5 = @(x) g5(x,c(i));
  f6 = @(x) g6(x,c(i));
  fcns = {f1, f2, f3, f4, f5, f6}; %f1, f2, f3, f4, f5, f6};
    ...
    for k = 1:length(methods)
      ...
      elseif k == 2
         ...
         tic, [xmin, fmin, exitflag, out] = fminbnd(@(x) f(x), a(j), b(j), ...
           optimset ('TolX',out_param.abstol / 2, 'MaxFunEvals', out_param.nmax, 'MaxIter', out_param.maxiter) ); t=toc;
      ...
      elseif k == 3
        try
          lastwarn('')
          chebf = chebfun(f,[a(j),b(j)],'splitting','on');
          tic, fmin = min(chebf); t=toc;
