...
\begin{equation}
\label{eqn:defn_mu}
\mu(f) := \Ex[f(\boldsymbol{X})] := \int_{[0,1]^d} f(\vx)\, \dif\vx, 
\end{equation}
...
\hmu(f) := w_0 + \sum_{i=1}^{n} f(\vx_i) w_i,
\end{equation}
where the weights, $w_0$, and  $\vw = (w_i)_{i=1}^n \in \reals^n$, and the nodes, $\{x_i\}_{i=1}^n \subset [0,1]^d$, are chosen to make the error, $\abs{\mu(f) - \hmu(f)}$, small.
...
Error analyis for cubature that assumes a deterministic integrand lying in a Banach space, leads to an error bound that is typically impractical for deciding how large $n$ must be to satisfy \eqref{eqn:err_crit}.  The deterministic error bound involves some (semi-)norm of the integrand, which is often more complex to compute than the original integral.
...
C(\vt,\vx)  = C(\vx, \vt),  \; \sum_{i,j = 1}^n a_iC(\vx_i,\vx_j) a_j  
> 0 \text{ when } \va \ne \vzero, \\
\forall n\in \mathbb{N}, \; \vt, \vx, (\vx_i)_{i=1}^n \in [0,1]^d, \;
\va = (a_i)_{i=1}^n \in \mathbb{R}^n.
\end{multline}
...
\frac{ \mCInv \vone \vone^T \mCInv }{\vone^T\mCInv \vone}
\right] \vy \times
(c_0 - \vc^T\mC^{-1}\vc)   \leq \errtol^2 
\biggr\}
.
\end{multline}

Finally, using the MLE estimated parameters $m_\MLE , s_\MLE$ and $\vthetaMLE$, the cubature rule:
...
\\
\\
\int_{\reals^d} f(\vx) \dvx & = \frac{2 \pi^{\frac d2}}{\text{gamma}(\frac d2)} \text{cosinteg}(d), \quad d=1,2,3,\hdots
\end{align*}
where
