...
In summary,  as shown in Figures \ref{fig:mvn-guaranteed-MLE} to \ref{fig:optprice-guaranteed-GCV}, the Bayesian cubature algorithm computes the integral within the user-specified threshold in nearly all of the test cases.  The exceptions sometimes occurred in the option pricing example for $\varepsilon=10^{-4}$. Our algorithm used maximum allowed sample size and still did not reach the stopping criterion $\err_{\textup{CI}} \leq \varepsilon$, due to the complexity and high dimension of the integrand. 
