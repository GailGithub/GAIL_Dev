...
The computational cost to compute $\mV^{(2n)T}\vb$ is then twice the cost of computing $\mV^{(n)T}\vb^{(1)}$ plus $2n$ multiplications plus $2n$ additions/subtractions.  An inductive argument establishes that $\mV^{(n)T}\vb$ requires only $\Order(n \log(n))$ operations.
