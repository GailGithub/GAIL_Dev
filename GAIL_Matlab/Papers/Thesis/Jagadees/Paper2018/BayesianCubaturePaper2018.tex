...
\begin{figure}
	...
	\end{subfigure}
	\caption{The $d=3$ multivarivate Gaussian density transformed to $d=2$ $f_{\text{Genz}}$.}
	\label{fig:MVN_Genz}
\end{figure}
...
\err_{\textup{GCV}} & =
\frac{2.58}{n}\sqrt{	\frac{\rlambda_1}{n + \rlambda_1} \sum_{i=2}^n \frac{\abs{\widetilde{y}_i}^2}{\lambda_i^2}  \left [ \frac 1n \sum_{i=1}^n \frac{1}{\lambda_i} \right]^{-1}} .
	\end{align}
\end{subequations}
...
Examples of this kind of transform include \cite{Sid08a}: \FJHNote{Do we need all of these periodizations?} \JRNote{Keeping only 3 varTxs}
\begin{align*}
...
\end{align*}
and again $\Phi$ is the standard normal distribution. \JRNote{Removed $a$ in $f_\textup{Keister}$}
%\begin{figure}
%	\captionsetup[subfigure]{labelformat=empty}
%	\begin{subfigure}[h]{0.48\linewidth}
