...
\paragraph{Multivariate Normal} does not have an analytic expression to compute the integral, So numerical algorithms are necessary to approximate it. But It's domain is $\mathbb{R}$. To integrate with our algorithm it has to be transformed to the unit cube.
We use the Genz's method for this reason as explained below. This method not only transforms the integrand into unit cube and also reduces the original dimension of the problem by 1.
\begin{figure}
	...
	\end{subfigure}
	\caption{Mutivariate Normal and  in d=2 and its transformed Genz function  $[0,1]^2$}
	\label{fig:MVN_Genz}
\end{figure}

\begin{multline*}
f_{\textup{MVN}} = \int_{[\va,\vb] \in \reals^d} \frac{\exp\bigl(- \frac 12 \vt^T \mSigma^{-1} \vt \bigr)}{\sqrt{(2 \pi)^d \det(\mSigma)}} \, \dvt 
\\
...
As we see from the figure, Genz function is not periodic, So we need to make it periodic (Section \ref{period_var_tx}) to get the best accuracy with Bayesian cubature.
...
\end{subfigure}
\caption{Multivariate Normal probability estimated using Matern kernel in d=2}
...
\subsection{Contribution of this research}
...
\end{prop}
\begin{proof}:
	consider any symmetric positive-definite shift-invariant-kernel of the form 
...
We have implemented this algorithm in MATLAB and is part of the Guaranteed Adaptive Integration Library (GAIL) as a method  \code{cubBayesLattice\_g}, for more details please refer to \cite{ChoEtal17b}. This algorithm uses the kernel defined in  \eqref{the_kernel_eqn_bernoulli} with order $r=2,4$.
...
\begin{multline}
\widehat{f}_\textup{Keister}(\vt) = (2\pi)^{\frac d2} a^d \cos(a\norm{ \Phi^{-1}(\vt)}) 
...
The following iterative algorithms describes the method to compute the true integral:
\begin{multline*}
\int_{\reals^d} f_{\textup{Keister}}(\vx) \dvx  = \frac{2 \pi^{\frac d2}}{\Gamma(\frac d2)} \text{CI}(d), \quad d=1,2,3,\hdots
\\
...
\begin{figure}
	...
	\caption[Keister guaranteed: Num sample]{Keister function integrated within $\varepsilon$ using Full Bayes stopping criterion}
...
\begin{figure}
	...
	\caption[Keister guaranteed: GCV]{Keister function integrated within $\varepsilon$ using Full Bayes stopping criterion and GCV}
...
\begin{figure}
	...
	\caption[Multivariate Normal guaranteed : Number of sample]{MVN integrated within $\varepsilon$ using Full Bayes stopping criterion}
...
As shown in the figures, Bayesian cubature algorithm computes the integral within  user specified threshold most of the times except on a few occasions. This is especially the case with option pricing example due to the complexity and high dimension of the integrand. 
...
Another noticeable aspect from the plots is how much estimated error differs from the true error. For option pricing example, estimated error is closer to the true error but it is not so much with Keister and multivariate Normal. This could be a future work to obtain more accurate error estimate.
