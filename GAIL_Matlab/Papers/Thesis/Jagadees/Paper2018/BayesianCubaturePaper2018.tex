...
Section \ref{sec:fast_BC}  introduces the concept of covariance kernels that match the nodes and expedites the computations required by our automatic Bayesian cubature. 
...
Rather than use maximum likelihood to determine $m$ and $s$ one can treat them as hyper-parameters and may chose a non-informative, conjugate prior, namely $\vrho_{m,s^2}(\xi, \lambda) \propto 1/\lambda$. Then the posterior density for the integral given the data using Bayes theorem is
\begin{align*}
...
\centering
\begin{subfigure}[h]{0.8\linewidth}
\includegraphics[width=1.1\linewidth]{MVN_rapid_time_Matern_d2_2018-Aug-31}
\end{subfigure}
...
Because the objective functions need to be computed multiple times in every iteration to find its minimum, to choose the optimal shape parameter. 
...
The generic automatic Bayesian cubature algorithm described in the previous section requires $\Order(n^3)$ operations to estimate $\vtheta$, compute the credible interval width, and compute the cubature.  There is also round-off error that can arise.  This section explains how to speed up the calculations and reduce this round-off error.  A key is to choose kernels that match the design, $\{\vx_i\}_{i=1}^n$, so that the vector-matrix operations required by Bayesian cubature can be accomplished using fast transforms at a cost of $\Order(n \log(n))$.
