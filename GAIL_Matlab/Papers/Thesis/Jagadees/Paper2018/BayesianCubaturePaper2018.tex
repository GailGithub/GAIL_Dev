...
\begin{gather} \label{vdCProp}
\phi(2i) = \phi(i)/2, \;  \phi(2i+1) = [\phi(i)+1]/2, \ \ \ i \in \natzero\\
 \label{vdCPropB}
\phi(i+n) = \phi(i) + 1/n, \qquad i = 0, \ldots, n-1,
\end{gather}
...
&= \mV^{(n)}\vb^{(1)} \quad \\
&+  \left(  \me^{2 \pi \sqrt{-1} \phi(i-1)} \sum_{j=1}^{n}  \me^{2 \pi n \sqrt{-1} \phi(i-1)\phi(j-1)} b_{n+j} \right)_{i=1}^n \\
& \hspace{5cm}  \text{by \eqref{vdCPropB}}\\
&= \mV^{(n)}\vb^{(1)} +  \left(  \me^{2 \pi \sqrt{-1} \phi(i-1)} \right)_{i=1}^n \odot \bigl(\mV^{(n)}\vb^{(2)} \bigr),
\end{align*}
where $\odot$ denotes the Hadamard (term-by-term) product.
...
We call \eqref{the_kernel_eqn_bernoulli} a Fourier kernel as the resulting Fast transform is the \emph{Fast Fourier transform} (FFT). Plots of this kernel in $d=$ is shown in \figref{fig:fourierkernel-dim1} with order $r=2,4$ and $\vtheta=0.2,0.8$.
