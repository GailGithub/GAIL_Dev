...
\end{align}
\end{subequations}
Where $\vone$ is a vector of all ones.
...
\end{subfigure}
\caption{Multivariate normal probability estimated using Mat\'ern kernel with $d=2$ using empirical stopping criterion. Top: Guaranteed integration within error tolerance $\varepsilon$. Bottom: Computation time rapidly increases with increase of $n$.}
\label{fig:MVN_Metern_d2b2}
\end{figure}

The results are in shown in \figref{fig:MVN_Metern_d2b2}. Sobol points are randomly scrambled in every run, causing the accuracy to vary accordingly. 
\JRNote{Do we need to explain why scrambling is needed?}
For each $\varepsilon$, the test was repeated 100 times. We can observe the algorithm meets the $\varepsilon$ 95\% of the times.
%with Intel i7 3630QM and 16GB RAM memory
On our test computer, it took more than an hour to compute $\hmu_n$ with $n=2^{14}$. As shown in \figref{fig:MVN_Metern_d2b2}, computation time increases rapidly with $n$. 
...
Not only the computational cost increases, but also the $\mC$ becomes highly ill-conditioned with increasing $n$.
...
Also notice the algorithm finished within 10 seconds of time for Keister and multivariate Normal. For the option pricing it look closer to 70 seconds. This is again due to the complexity of the integrand.
