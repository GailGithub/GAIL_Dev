...
\hmu_\MLE  &= 
\left(
\frac{ (1 - \vone^T  \mCInv\vc ) }{ \vone^T \mCInv \vone}   \vone +  \vc 
\right)^T  \mCInv \vy, \\
\nonumber 
\hsigma_\MLE^2 & : = 
 \vy^T \left[ \mCInv - 
...
(c_0 - \vc^T\mC^{-1}\vc ),
\end{align}
So the stopping criterion for the empirical Bayes case is
\begin{multline} \label{eqn:stopcritMLE}
n  = \min \biggl \{n_j \in \posIntegers:  
2.58^2  \vy^T \left[ \mCInv - 
\frac{ \mCInv \vone \vone^T \mCInv }{\vone^T\mCInv \vone}
\right] \vy 
\\
\times 
(c_0 - \vc^T\mC^{-1}\vc ) \vy \; \le \; \varepsilon^2 \biggr\}.
\end{multline}
...
& \times \int_{-\infty}^\infty  \exp \biggl(
...
& \propto \displaystyle \int_{0}^\infty  \frac{1}{\lambda^{(n+3)/2}} \\
&\times \qquad \qquad \int_{-\infty}^\infty  \exp\left( -\frac{\alpha \xi^2 -2 \beta \xi + \gamma}{2\lambda(c_0  -\vc ^T \mC^{-1} \vc)} \right) \, \D \xi \D \lambda, \\
\intertext{where}
...
\begin{align*}
\MoveEqLeft[1]{\alpha \gamma - \beta^2} \\
& = \vone^T \mC^{-1} \vone (c_0  -\vc ^T \mC^{-1} \vc) (z - \vc^T \mC^{-1} \vy )^2 \\
& \quad - 2 \vone^T \mC^{-1} \vy (c_0  -\vc ^T \mC^{-1} \vc)(1 - \vc^T \mC^{-1} \vone)(z - \vc^T \mC^{-1} \vy ) \\
& \quad  + (1 - \vc^T \mC^{-1} \vone)^2 \vy^T \mC^{-1} \vy (c_0  -\vc ^T \mC^{-1} \vc) \\
&\quad   + [\vone^T \mC^{-1} \vone \vy^T \mC^{-1} \vy - (\vone^T \mC^{-1} \vy)^2](c_0  -\vc ^T \mC^{-1} \vc)^2  \\
& \propto \vone^T \mC^{-1} \vone  \left (z - \vc^T \mC^{-1} \vy - \frac{(1 - \vc^T \mC^{-1} \vone)\vone^T \mC^{-1} \vy}{\vone^T \mC^{-1} \vone } \right )^2 \\
& \quad  -  \frac{[(1 - \vc^T \mC^{-1} \vone)\vone^T \mC^{-1} \vy]^2}{\vone^T \mC^{-1} \vone }  \\
& \quad + (1 - \vc^T \mC^{-1} \vone)^2 \vy^T \mC^{-1} \vy \\
& \quad  (c_0  -\vc ^T \mC^{-1} \vc) [\vone^T \mC^{-1} \vone  \vy^T \mC^{-1} \vy
- (\vone^T \mC^{-1} \vy)^2] \\
& \propto \left (z - \left[ \frac{(1 - \vc^T \mC^{-1} \vone)\vone}{\vone^T \mC^{-1} \vone } + \vc \right]^T \mC^{-1} \vy \right )^2 \\
& \quad  + \left[\frac{(1 - \vc^T \mC^{-1} \vone)^2}{\vone^T \mC^{-1} \vone} + (c_0  -\vc ^T \mC^{-1} \vc) \right] \\
& \quad \times \vy^T\left[ \mC^{-1} 
...
n  = \min \biggl \{n_j \in \posIntegers:  
\\
\frac {t_{n_j-1,0.995}^2}{n_j - 1}   \left[\frac{(1 - \vc^T \mC^{-1} \vone)^2}{\vone^T \mC^{-1} \vone} + (c_0  -\vc ^T \mC^{-1} \vc) \right]
\\
 \times \vy^T \left[\mC^{-1} - \frac{\mC^{-1} \vone \vone^T \mC^{-1}}{\vone^T \mC^{-1} \vone}  \right] \vy  \le \varepsilon^2 \biggr\}.
\end{multline}
is more conservative than that in the empirical Bayes case, \eqref{eqn:stopcritMLE}.
Here $t_{n-1,0.995}$ denotes the $99.5$ percentile of a standard Student's $t$-distribution with $n-1$ degrees of freedom.

Because the shape parameter, $\vtheta$, enters the definition of the covariance kernel in a non-trivial way, it typically cannot be practically considered as a hyper-parameter in a full Bayesian model.  The empirical Bayes approach for determining $\vtheta$ in \eqref{eqn:thetaMLE} is one possibility
...
Another alternative to determining $m$, $s$, and $\vtheta$ is \emph{leave-one-out cross-validation}.  Let $\widetilde{y}_i$ denote the expected value of $f(\vx_i)$ conditioned on $\vf_{-i} = \vy_{-i}$, where the subscript $-i$ denotes vector excluding the $i^{\text{th}}$ component.  The cross-validation criterion, which is to be minimized, is
\begin{equation} \label{FJH:eq:CVA}
\textup{CV} = \sum_{i=1}^n (y_i - \widetilde{y}_i)^2.
\end{equation}

Let $\mA = \mC^{-1}$ and $\vzeta = \mA (\vy - m \vone)$, and partition $\mC$, $\mA$, and $\vzeta$ as
\begin{align*}
\mC = \begin{pmatrix} C_{ii}  & \vC_{-i,i}^T \\  \vC_{-i,i} & \mC_{-i,-i}\end{pmatrix}, 
\mA = \begin{pmatrix} A_{ii}  & \vA_{-i,i}^T \\  \vA_{-i,i} & \mA_{-i,-i}\end{pmatrix},  
\vzeta = \begin{pmatrix} \zeta_i   \\  \vzeta_{-i} \end{pmatrix}
\end{align*}
Then using the results from \cite{RasWil06a} Gaussian Identities eqn A.6, implies that 
\begin{align*}
...
Note that
	...
	\end{gather*}
%
% Reference: https://stats.stackexchange.com/questions/30588/deriving-the-conditional-distributions-of-a-multivariate-normal-distribution
%
