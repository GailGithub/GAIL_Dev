...
\begin{figure}
	...
	\end{subfigure}
	\caption{The original multivarivate Gaussian density (left) \FJHNote{Check the left plot and label the axes.  Did you use your $\mL$ from below? Which dimension?}  and the transformed integrand, $f_{\text{Genz}}$ for $d=3$ (right).}
	\label{fig:MVN_Genz}
\end{figure}


We use the following parameter values in the simulation: \FJHNote{Delete $d=4$ if you don't use it.}
\begin{equation*}
\begin{array}{c|ccc}
& a  & b & L  \\
\hline
...
We have implemented the fast adaptive Bayesian cubature algorithm in MATLAB as part of the Guaranteed Adaptive Integration Library (GAIL) \cite{ChoEtal17b} as \allowbreak \code{cubBayesLattice\_g}. This algorithm uses the kernel defined in  \eqref{the_kernel_eqn_bernoulli} with  $r=1,2$.  The rank-1 lattice node generator is taken from \FJHNote{\cite{???}}.
...
Examples of this kind of transform include \cite{Lau96a}, \cite{Sid93}, \cite{Sid08a}: \FJHNote{Do we need all of these periodizations?}
\begin{align*}
