...
\begin{abstract}
Automatic cubatures approximate multidimensional integrals to user-specified error tolerances.  For large dimensional problems, the sampling density may be fixed, but the
sample size, $n$, is determined automatically. Bayesian cubature postulates that the integrand is an instance of a stochastic process.  In our case, we assume a Gaussian process parameterized by a constant mean and a covariance function defined by a scale parameter and a function specifying how the integrand values at two different points in the domain are related.
The hyper-parameters are estimated from integrand values or are given non-informative priors.  This leads to a data-driven error bound on the cubature.  

However, the process just outlined requires vector-matrix operations  with a computational cost of $O(n^3)$, where   $n$ is the number of integrand values. Our innovation is to pair low discrepancy data sites with matching kernels that lower the  computational cost to $O(n \log n)$.  The credible interval for the Bayesian posterior error is used to choose $n$ automatically to satisfy the user-defined error tolerance.  This approach is demonstrated using rank-1 lattice sequences and shift-invariant kernels.
...
$ \mu := \int_{\reals^d} g(\vx) \, \dif \vx$ where $\mu$ has no closed form analytic expression. Typically, $g$ is only accessible through a black-box function routine. 
...
Hickernell and Jim\'enez-Rugama~\cite{HicJim16a,JimHic16a} have developed stopping criteria for cubature rules based on low discrepancy nodes by tracking the discrete Fourier coefficients of the integrand.  The algorithm proposed here also depends on the discrete Fourier coefficients of the integrand, but in a different way.  Although we do not explore automatic Bayesian cubature for relative error tolerances, the recent work by Hickernell, Jim\'enez-Rugama, and Li~\cite{HicEtal17a} suggest how this might be done.
...
\\ \qquad \text{where } \mC = \left(  C_\vtheta(\vx_i,\vx_j)  \right)_{i,j=1}^n,\\
...
To demonstrate automatic Bayesian cubature consider the Matern kernel as covariance function:
\begin{align}
\label{matern_kernel}
C_{\theta}(\vx, \vt) = \prod_{k=1}^d \exp(-\theta|\vx_k-\vt_k|)(1+\theta |\vx_k-\vt_k|)
\end{align}

\paragraph{Multivariate Normal}\label{MVN_example} does not have an analytic expression to compute the integral, So numerical algorithms are necessary to approximate it. But It's domain is $\mathbb{R}$. To integrate with our algorithm it has to be transformed to the unit cube.
We use the Genz's method for this reason. This method not only transforms the integrand into unit cube but also reduces the original dimension of the problem by 1.
\begin{figure}
	...
	\end{subfigure}
	\caption{Mutivariate Normal in d=2 and its transformed form of Genz function  $[0,1]^2$}
	\label{fig:MVN_Genz}
\end{figure}
\begin{multline*}
...
The generic automatic Bayesian cubature algorithm described in the previous section requires $\Order(n^3)$ operations to estimate $\vtheta$, compute the credible interval width, and compute the cubature.  There is also the potential ill-condtioning that can arise.  This section explains how to speed up the calculations and avoid ill-conditioning.  A key is to choose kernels that match the design, $\{\vx_i\}_{i=1}^n$, so that the vector-matrix operations required by Bayesian cubature can be accomplished using fast transforms at a cost of $\Order(n \log(n))$.
...
&= \mV^{(n)}\vb^{(1)}  +  \biggl(  \me^{-\pi \sqrt{-1} \phi(i-1)}  \\
...
\frac{2.58}{n^{1/2}}\sqrt{	\frac{\rlambda_1}{n + \rlambda_1} \sum_{i=2}^n \frac{\abs{\widetilde{y}_i}^2}{\lambda_i^2}  \left [ \frac{1}{n + \rlambda_1} + \sum_{i=2}^n \frac{1}{\lambda_i} \right]^{-1}} .
	\end{align}
\end{subequations}
Since $\rlambda_1 = \lambda_1 - n$ and $\lambda \sim n$ it follows $\lambda_1$ is a small value closer to 1. For significantly large $n$, credible intervals $\err_\MLE$ and $\err_\full$ are almost the same, since the Student-t distribution quantile value $t_{n_j-1,0.995}$ approaches $2.58$. Whereas to compare $\err_\GCV$, need little more insight due to the term $\lambda_i^2$ in the denominator. Rewriting it:
\begin{align*}
\err_\GCV^2 & =
...
\begin{prop}
Any periodic symmetric positive-definite shift-invariant-kernel of the form \eqref{eq:shInv} satisfying properties \eqref{addAssump} that can be matched with rank-1 Lattice data-sites, is a fast-transform-kernel \eqref{fastcompAssump}.
Thus it has a fast-transform with computational cost of $\Order(n \log n )$ in the form of \emph{Fast Fourier transform}(FFT). Consequently the kernel can be used to construct fast adaptive Bayesian cubature with parameter estimation \eqref{thetaSimple}, stopping criteria \eqref{fastStoppingCriterions} and cubature rule \eqref{muhatGCV-FB-MLE-Simple}.
\end{prop}


\paragraph{MATLAB program:}
We have implemented the fast adaptive Bayesian cubature algorithm in MATLAB and is part of the Guaranteed Adaptive Integration Library (GAIL) as a method named \code{cubBayesLattice\_g}, for more details please refer to \cite{ChoEtal17b}. This algorithm uses the kernel defined in  \eqref{the_kernel_eqn_bernoulli} with order $r=2,4$.
...
Having all the tools and optimization done handy, now we shall run the numerical simulations to see the performance. \code{cubBayesLattice\_g} is used to run these tests.

We used three functions 1. Multivariate Normal, 2. Keister function and 3. Option pricing to test the integration speed and accuracy of \textit{Fast Bayesian cubature algorithm}. A random shift of $\delta \sim \mathcal{U}[0,1)^d$ is added to the rank-1 Lattice points. This provides randomness in the cubature's behavior. The test is run for 100 times and each result is plotted against $\frac{\abs{\mu - \hmu}}{\varepsilon}$. The tests are repeated with four different error thresholds $\varepsilon \in \{10^{-1}, 10^{-2}, 10^{-3}, 10^{-4}\}$ and three stopping criteria we developed in this research. We use $\frac{\abs{\mu - \hmu}}{\varepsilon}$ in x-axis to enable the plot to include all the tests with different $\varepsilon$s. 
...
This function was already introduced in Section \ref{MVN_example}. We use the variable transformed function $f_{\textup{Genz}}$ as our algorithm needs the integrand to be in unit cube.
\begin{figure}
	\captionsetup[subfigure]{labelformat=empty}
	\centering
	\begin{subfigure}[h]{0.48\linewidth}
		\includegraphics[width=1.1\linewidth]{GenzFunc_varTx_C0}
	\end{subfigure}
	\caption{$f_{\textup{GenzP}}$: $C^0$ Periodization transformed Genz function.}
	\label{fig:GenzP}
\end{figure}
As we see from \figref{fig:MVN_Genz}, Genz function is not periodic, So we need to also make it periodic (\secref{period_var_tx}) to get the best accuracy with our algorithm. We used $C^0$ periodization-transformed $f_{\textup{GenzP}}(\vx) = f_{\textup{Genz}}(\vPsi(\vx)) \prod_{\ell = 1}^d \Psi'(x_l)$ in the simulations. One can compare \figref{fig:MVN_Genz} with \figref{fig:GenzP} to see the effect of the periodization transform.
The simulation results for this example function are summarized in figures \figref{fig:mvn-guaranteed-MLE}, \figref{fig:mvn-guaranteed-FB} and \figref{fig:mvn-guaranteed-GCV}. We used $d=3, r=2$ and $C2sin$ periodization transform.

\begin{figure}
	\centering
	\includegraphics[width=0.95\linewidth]{"MVN_guaranteed_time_MLE_C2sin_d3_r4_2018-Aug-19"}
	\caption[Guaranteed:]{Multivariate Normal integrated within $\varepsilon$ using empirical Bayes stopping criterion using $d=3, r=2$ and Sidi's $C^2$ transform}
	\label{fig:mvn-guaranteed-MLE}
\end{figure}
\begin{figure}
	\centering
	\includegraphics[width=0.95\linewidth]{"MVN_guaranteed_time_full_C2sin_d3_r4_2018-Aug-19"}
	\caption[MVN guaranteed : FB]{Multivariate Normal integrated within $\varepsilon$ using full-Bayes stopping criterion using $d=3, r=2$ and Sidi's $C^2$ transform}
	\label{fig:mvn-guaranteed-FB}
\end{figure}
\begin{figure}
	\centering
	\includegraphics[width=0.95\linewidth]{"MVN_guaranteed_time_GCV_C2sin_d3_r4_2018-Aug-19"}
	\caption[MVN guaranteed : GCV]{Multivariate Normal integrated within $\varepsilon$ using GCV stopping criterion using $d=3, r=2$ and Sidi's $C^2$ transform}
	\label{fig:mvn-guaranteed-GCV}
\end{figure}

\item Keister function

This multidimensional integral function is based on the paper \cite{Kei96}, inspired by a physics application. 
\begin{figure}
	\captionsetup[subfigure]{labelformat=empty}
	\begin{subfigure}[h]{0.48\linewidth}
...
\end{figure}
\begin{align}
\label{keisterFunction}
g_{\textup{Keister}}(\vx) =  \cos(\norm{ \vx}) \exp(-\norm{ \vx }^2) \dvx
\end{align}
The domain of this function is $\reals^d$ so variation transformation must be used to transform this to $[0,1]^d$. 
\begin{multline*}
\int_{\reals^d} \cos(\norm{ \vx}) \exp(-\norm{ \vx }^2)  \dvx  
\\ = 
\int_{\reals^d} \cos(a\norm{ \vx}) \exp(-a\norm{ \vx }^2)  a^d \dvx 
\\ =
(2\pi)^{\frac d2} a^d \int_{[0,1]^d} \cos(a\norm{ \Phi^{-1}(\vt)})
\\
\times \exp\left(-(a^2-\frac{1}{2})\norm{ \Phi^{-1}(\vt) }^2\right)  \dvt
\end{multline*}
So the transformed function is
\begin{multline}
{f}_\textup{Keister}(\vt) = (2\pi)^{\frac d2} a^d \cos(a\norm{ \Phi^{-1}(\vt)}) 
\\ \times \exp(-(a^2-\frac{1}{2})\norm{ \Phi^{-1}(\vt) }^2)
\end{multline}
Where $a > $
To enforce additional boundary conditions, we could use any desired periodizing variable transforms listed in Section \ref{period_var_tx}.
The following iterative technique used to compute the true integral:
\begin{multline*}
\int_{[0,1]^d} f_{\textup{Keister}}(\vx) \dvx  = \frac{2 \pi^{\frac d2}}{\Gamma(\frac d2)} \text{CI}(d), \quad d=1,2,3,\hdots
\\
\text{where}, \Gamma  := \text{gamma function}, \; \text{and}
\\
I_s(1) = \frac{\sqrt{\pi}}{2 \exp(1/4)}, 
\\
I_s(1) = \int_{x=0}^\infty \exp(-\vx^T\vx)\sin(\vx) \dvx 
\\
\qquad =  0.4244363835020225,
\\
I_c(2) = \frac{1-I_s(1)}{2}, \;
I_s(2) = \frac{I_c(1)}{2}
\\
...
\begin{figure}
	...
	\caption[Keister guaranteed:MLE]{Keister function integrated within $\varepsilon$ using empirical Bayes stopping criterion using $d=4, r=2$ and $C^1$ transform}
	\label{fig:keister-guaranteed-MLE}
\end{figure}
\begin{figure}
	\centering
	\includegraphics[width=0.95\linewidth]{"Keister_guaranteed_time_full_C1_d4_r4_2018-Aug-19"}
	\caption[Keister guaranteed:FB]{Keister function integrated within $\varepsilon$ using full-Bayes stopping criterion using $d=4, r=2$ and $C^1$ transform}
	\label{fig:keister-guaranteed-FB}
\end{figure}
\begin{figure}
	\centering
	\includegraphics[width=0.95\linewidth]{"Keister_guaranteed_time_GCV_C1_d4_r4_2018-Aug-19"}
	\caption[Keister guaranteed:GCV]{Keister function integrated within $\varepsilon$ using GCV stopping criterion using $d=4, r=2$ and $C^1$ transform}
	\label{fig:keister-guaranteed-GCV}
\end{figure}
The Figures \ref{fig:keister-guaranteed-MLE}, \ref{fig:keister-guaranteed-FB} and \ref{fig:keister-guaranteed-GCV} summarizes the result for this example function. 
For these tests we used $C^1$ periodization-transformed function $f_{\textup{KeisterP}}(\vx) = f_{\textup{Keister}}(\vPsi(\vx)) \prod_{\ell = 1}^d \Psi'(x_l)$, dimension $d=4$ and Bernoulli order $r=2$.
As we can see the stopping criteria is very conservative so the true errors are much smaller than user requested. This could be attributed to the smoothness of the variable transformed ${f}_\textup{Keister}(\vt)$.
...
\ref{fig:optprice-guaranteed-GCV} summarizes the results for this example. We used $d=12, r=2$ and $Baker$ periodization transform for this example.

\begin{figure}
	\centering
	\includegraphics[width=0.95\linewidth]{"optPrice_guaranteed_time_MLE_Baker_d12_r2_2018-Aug-19"}
	\caption[Option pricing Guaranteed: MLE]{Option pricing estimated within $\varepsilon$ using empirical Bayes stopping criterion using $d=12, r=1$ and Baker transform}
...
\begin{figure}
	...
	\caption[OptPrice guaranteed : FB]{Option pricing estimated within $\varepsilon$ using Full Bayes stopping criterion using $d=12, r=1$ and Baker transform}
...
\begin{figure}
	...
	\caption[OptPrice guaranteed : GCV]{Option pricing estimated within $\varepsilon$ using GCV stopping criterion using $d=12, r=1$ and Baker transform}
...
We developed an automatic Bayesian cubature algorithm that can estimate the integral within user defined error threshold. To accomplish the automatic stopping we provided three different stopping criteria 1. Maximum like estimated based, 2. Full Bayes and 3. Generalized cross validation stopping criterion.
We demonstrated the bare-bone automatic Bayesian cubature algorithm is computationally intensive and may not be suitable for practical applications without speedup.
We developed and generalized a technique called \emph{Fast transform kernel}. Using this we further developed fast automatic Bayesian cubature algorithm that has very low computational cost in the order of $\Order(n \log n)$ compared to direct Bayesian cubature of $\Order(n^{3})$. 
We provided a sample implementation of the proposed fast algorithm using a shift invariant family of kernels based on Bernoulli polynomials.
By adjusting the Bernoulli order $r$ of the kernel and using the appropriately smoother variable transformation, one could get higher order of error convergence. 
In general, without any assumption on the integrand's mean, the optimal $\hmu$ is just the sample mean and so the order of convergence is defined by the smoothness of the underlying function that is integrated and the variable transformation used. 
Though the optimal $\hmu$ is just the sample mean, the error bound $\errn$ still depends on the kernel order.
With higher order $r$ shift invariant kernel, the error bound $\errn$ closely matches the actual error.
We could integrate using upto $2^{23}$ data points on a 16GB of RAM memory computer within 2 minutes. 
Numerical results show the theoretical error $\errn$ closely matches the actual error but it could still be improved with more tighter error bound especially when the $r$ is smaller.
...
Using the established generalized requirements for a {fast-transform-kernel}, we could use the same approach with other point-sets and matching kernels to achieve similar or better performance and accuracy. One such point-sets that to consider in future is, \textit{Sobol points} and with appropriate choice of kernel, which should lead to \textit{Fast Walsh Transform}.
