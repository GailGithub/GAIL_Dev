...
function res=TestcubMCgRELTOLDiffSettings(test,fun,param)
tstartwhole=tic;
...
if any(strcmp('cubLattice',test.whichsample))
    ...
    res.Latticeoutcone = tempinitial;
...
for irep=1:test.nrep
    ...
    if strcmp(fun.funtype,'gaussianker')
        ...
        res.tol(irep) = max(abs(param.exactintegral)*param.reltol,param.abstol);
    end
    if strcmp(fun.funtype,'Keister')
        res.exactintegral(irep) = param.exactintegral;
        res.tol(irep) = max(abs(param.exactintegral)*param.reltol,param.abstol);
    ...
    if any(strcmp('cubSobol',test.whichsample))
        ...
        res.Soboloutcone(irep) = sum(out_param.exitflag);
        res.SobolQ(irep)=q;
    ...
    if any(strcmp('cubLattice',test.whichsample))
        ...
        res.Latticeoutcone(irep) = sum(out_param.exitflag);
        res.LatticeQ(irep)=q;
    ...
    if param.dim==1
        if any(strcmp('integral',test.whichsample))
            ...
            res.integralQ(irep)=integral(@(x) transpose(testfun(x')),...
                param.interval(1),param.interval(2),...
                'AbsTol',param.abstol,'RelTol',param.reltol);
