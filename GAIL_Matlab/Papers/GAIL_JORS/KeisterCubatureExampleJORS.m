...
function [succTable,avgAbsErrTable,timeTable,nSampleTable,warnTable] = KeisterCubatureExampleJORS(nRep) 
...
for i=1:nd
    ...
    for k = 1:nRep
        ...
        inputArgs =[inputArgs {'order',1, 'ptransform','C1','arbMean',true}];
...
%% Finally we do BayNetes Net
IBayNetvec(nRep,nd) = 0; %vector of answers
timeBayNet(nRep,nd) = 0;
nSampleBayNet(nRep,nd) = 0;
nWarnBayNet(nRep,nd) = 0;
tic
for i=1:nd
    d = dvec(i);
    for k = 1:nRep
        ...
        [IBayNetvec(k,i), out] = compInteg(obj);
        if out.exitflag > 0
            nWarnBayNet(k,i) = 1;
        end
       timeBayNet(k,i) = out.time;
       nSampleBayNet(k,i) = out.n;
   end
end
disp(' ')
toc
timeBayNet = mean(timeBayNet)
nSampleBayNet = mean(nSampleBayNet)
...
absErrBayNet = abs(Ivec-IBayNetvec);
succBayNet = mean(absErrBayNet <= repmat(abstol,nRep,1))
avgAbsErrBayNet = mean(absErrBayNet)
...
disp(['warnings issued by cubBayesNet_g: ', num2str(sum(nWarnBayNet,1))])

outFileName = gail.save_mat('GAIL_JORS2021_Output',['KeisterCubExPearcDataNRep' int2str(nRep)],...
    true, dvec, abstol, ...
    avgAbsErrMC, avgAbsErrLat, avgAbsErrSob, avgAbsErrBay, avgAbsErrBayNet, ...
    succMC, succLat, succSob, succBay, succBayNet, ...
    timeMC, timeLat, timeSob, timeBay, timeBayNet, ... 
    nSampleMC, nSampleLat, nSampleSob, nSampleBay, nSampleBayNet, ...
    warnMC, warnLat, warnSob, warnBay, warnBayNet);

KeisterCubExJorsOut(outFileName)

%% Prepare outputs
rowNames = {'cubMC_g','cubLattice_g','cubSobol_g','cubBayesLattice_g', 'cubBayesNet_g'};
colNames = {'d_eq_3','d_eq_8'};
succTable = array2table([succMC; succLat; succSob; succBay; succBayNet],'RowNames',rowNames,'VariableNames',colNames);
%succTable(1,1)
avgAbsErrTable = array2table([avgAbsErrMC; avgAbsErrLat; avgAbsErrSob; avgAbsErrBay; avgAbsErrBayNet],'RowNames',rowNames,'VariableNames',colNames);
timeTable = array2table([timeMC; timeLat; timeSob; timeBay; timeBayNet],'RowNames',rowNames,'VariableNames',colNames);
nSampleTable = array2table([nSampleMC; nSampleLat; nSampleSob; nSampleBay; nSampleBayNet],'RowNames',rowNames,'VariableNames',colNames);
warnTable = array2table([warnMC; warnLat; warnSob; warnBay; warnBayNet],'RowNames',rowNames,'VariableNames',colNames);
