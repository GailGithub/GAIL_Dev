...
mmax = 22; % Maximum budget permitted in the algorithm in the form 2^mmax. It depends on your computer memory
abstol = 1e-3; % Absolute error tolerance
reltol = 0; % Relative error tolerance. If 0, we have pure absolute error.

%% Dumb test with cov = Id (to check real error)
mu = zeros(d,1);
cov = eye(d);

factor = 3.5;
hyperbox = [-factor*ones(1,d) ; factor*ones(1,d)]; % Integration limits
[approx_prob1,out_param1] = multi_normcdf(hyperbox,mu,cov,abstol,reltol,mmax); % Solution approx_prob and integration output parameters in out_param
disp('---------- Error checking cubSobol_g example 1')
disp(['Solution for d=' num2str(d) ', cubSobol_g : ' num2str(approx_prob1)])
disp(['Real error was ' num2str(abs((gail.stdnormcdf(factor)-gail.stdnormcdf(-factor))^d-approx_prob1)) ' which is less than the tolerance ' num2str(abstol) '. Time was ' num2str(out_param1.time) ' seconds.'])
...
hyperbox = [-Inf*ones(1,d) ; sqrt(d)*rand(1,d)]; % Integration limits
[approx_prob2,out_param2] = multi_normcdf(hyperbox,mu,cov,abstol,reltol,mmax); % Solution approx_prob and integration output parameters in out_param
[exact_p2 , e_out2] = cubSobol_g(@(t) prod(gail.stdnormcdf(bsxfun(@plus, hyperbox(2,:),sqrt(sig)*t)/sqrt(1-sig)),2),[-Inf;Inf],'normal',abstol/10^3,0); % Computing the real solution
disp('---------- Error checking cubSobol_g example 2')
disp(['Solution for d=' num2str(d) ', cubSobol_g : ' num2str(approx_prob2)])
disp(['Real error was ' num2str(abs(exact_p2-approx_prob2)) ' which is less than the tolerance ' num2str(abstol) '. Time was ' num2str(out_param2.time) ' seconds.'])
...
[approx_prob3,out_param3] = multi_normcdf(hyperbox,mu,cov,abstol,reltol,mmax); % Solution approx_prob and integration output parameters in out_param
