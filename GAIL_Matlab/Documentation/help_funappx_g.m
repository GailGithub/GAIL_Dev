...
%
% pp = *funappx_g*(f,a,b,abstol,taulo,tauhi,nmax) |for a given
...
f = @(x) exp(-100*x.^2); [pp, out_param] = funappx_g(f)
...
[pp, out_param] = funappx_g(@(x) exp(-100*x.^2),0,10,1e-7,10,100)
...
[pp, out_param] = funappx_g(@(x) exp(-100*x.^2), in_param)
