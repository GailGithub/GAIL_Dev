...
warning('off','GAIL:funappxglobal_g_CSC:peaky')
warning('off','GAIL:funappxglobal_g_CSC:exceedbudget')
for i = 1:nrep;
    a = 0;
    b = c(i)+1;
    f1 = @(x) (x-c(i)).^2;
...
%         - 2* sin(x) - c(i)*sin(2*x)));
    ...
    mem(1,1,i) = out_param.bytes;
    tic;
    [~, out_param] = funappxglobal_g_CSC(f1,a,b,abstol,nlo,nhi);
    t=toc;
    time(1,2,i) = t;
    npoints(1,2,i) = out_param.npoints;
    mem(1,2,i) = out_param.bytes;
    tic;
    [~, out_param] = funappx_g(f2,a,b,abstol,nlo,nhi);
    t=toc;
    time(2,1,i) =  t;
    npoints(2,1,i) = out_param.npoints;
    mem(2,1,i) = out_param.bytes;
    tic;
    [~, out_param] = funappxglobal_g_CSC(f2,a,b,abstol,nlo,nhi);
    t=toc;
    time(2,2,i) =  t;
    npoints(2,2,i) = out_param.npoints;
    mem(2,2,i) = out_param.bytes;
    ...
    mem(3,1,i) = out_param.bytes;
    tic;
    [~, out_param] = funappxglobal_g_CSC(f3,a,b,abstol,nlo,nhi);
    t=toc;
    time(3,2,i) =   t;
    npoints(3,2,i) = out_param.npoints;
    mem(3,2,i) = out_param.bytes;
...
%     tic;
%     [~, out_param] = funappxglobal_g_CSC(f4,a,b,abstol,nlo,nhi);
%     t=toc;
%     time(4,2,i) =   t;
%     npoints(4,2,i) = out_param.npoints;
%     tic;
end;
warning('on','GAIL:funappxglobal_g_CSC:exceedbudget')
warning('on','GAIL:funappx_g:peaky')
warning('on','GAIL:funappx_g:exceedbudget')
warning('on','GAIL:funappxglobal_g_CSC:peaky')
...
memratio = zeros(nrep,n);
for i=1:nrep;
    for j=1:n;
        timeratio(i,j) = time(j,1,i)/time(j,2,i);
        npointsratio(i,j) = npoints(j,1,i)/npoints(j,2,i); 
        memratio(i,j) = mem(j,1,i)/mem(j,2,i); 
    end
end
 
timeratio = sort(timeratio(:));
npointsratio = sort(npointsratio(:));
memratio = sort(memratio(:));

%% Output the table
% To just re-display the output, load the .mat file and run this section
% only
display(' ')
display('   Test      Number of Points       Time Used             Memory (bytes) Used')
display(' Function   Local      Global     Local       Global          Local    Global')
npointslgratio = zeros(1,n);
timelgratio = zeros(1,n);

for i=1:n
    display(sprintf('%9.0f %9.0f  %9.0f %11.7f  %11.7f %11.0f  %11.0f',...
        [i mean(npoints(i,1,:)) mean(npoints(i,2,:)) mean(time(i,1,:)) mean(time(i,2,:)) mean(mem(i,1,:)) mean(mem(i,2,:))])) 
...
idx=find(memratio<1);
max_idx_n = max(idx);
memratio(1:max_idx_n) = 1.0 ./memratio(1:max_idx_n);
...
if usejava('jvm') || MATLABVERSION <= 7.12
    ...
    t =1:nrep*n;
    plot(t,timeratio,'r',t,npointsratio,'b:',t,memratio,'g--');
    legend('time ratio','points ratio', 'memory ratio');
    gail.save_eps('WorkoutFunappxOutput', 'WorkoutFunAppxTest');
