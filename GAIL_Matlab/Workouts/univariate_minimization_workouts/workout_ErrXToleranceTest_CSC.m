...
n = nrep;
if (n >= 100)
    warning('off','GAIL:funmin_g_CSC:exceedbudget');
    warning('off','GAIL:funmin_g_CSC:peaky');
end;
a = 10.^(-4+3*rand(n,1));
...
warning('on','GAIL:funmin_g_CSC:exceedbudget');
warning('on','GAIL:funmin_g_CSC:peaky');
...
%% Save output
gail.save_mat('WorkoutFunminOutput', 'workout_ErrXToleranceTest',true,tauvec,prob,ntau);
