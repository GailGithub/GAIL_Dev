% TEST_CUBLATTICE_G This is the driver script to test cubLattice_g algorithm
%using seven integrands of dimensions up to 8
%clear all;close all;clc;
function [ut_abserr,ut_relerr,abstol,reltol] = Test_cubLattice_gCLASS
%[
dimsize = 3;   %8
indexsize = 2; %5
abstol = 1e-3;
...
for dim=1:dimsize
  ...
  w.domain = [startingpoint;endingpoint];% the integration interval
  w.absTol = abstol;% the absolute tolerance
  w.relTol = reltol;% the relative tolerance
  ...
  w.inflateFun = @(m) 10*2.^-m;% standard deviation inflation factor
  ...
  for index=1:indexsize % index refers to different integrands in genz_test_fun
    w.f = @(x)genz_test_fun(x,index,in_param.dim,alpha,beta,r);
    % the test function
    f_true = genz_test_fun_true (w.domain,index,in_param.dim,alpha,beta,r);
