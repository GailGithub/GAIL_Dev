...
if output.MCconverge
    if notamer;
        ...
        h=semilogx(Nplot,price.callN(Nplot),'-b',Nplot,price.putN(Nplot),'r-', ...
            [1 sample.N],exprice.gmeancall*[1 1],'--b',[1 sample.N],exprice.gmeanput*[1 1],'--r');  
        set(h,'linewidth',2);
        xlabel('Number of Stock Paths')
        ylabel('Option Price')
        legend('Call','Put','Location','Best')
        set(gca,'Xtick',10.^(1:floor(log10(sample.N))))
        eval(['print -depsc Conv' sample.type 'Sample' option.type 'Price' int2str(sample.N) '.eps'])
        figure; 
        h=loglog(Nplot,abs(exprice.gmeancall-price.callN(Nplot))/exprice.gmeancall,'-b',Nplot,abs(exprice.gmeanput-price.putN(Nplot))/exprice.gmeanput,'r-');  
        set(h,'linewidth',2);
        xlabel('Number of Stock Paths')
        ylabel('Relative Error of Option Price')
        legend('Call','Put','Location','Best')
        set(gca,'Xtick',10.^(1:floor(log10(sample.N))))
        eval(['print -depsc Err' sample.type 'Sample' option.type 'PriceN' int2str(sample.N) 'd' int2str(sample.d) '.eps'])
        refresh
    else
...
if output.numer
    ...
    if asset.ncontrol==0;
        if notamer; 
            disp(['The call price is $' num2str(price.callN(sample.N),'%0.4f') char(177) price.callerr])
            disp(['   and the put price is $' num2str(price.putN(sample.N),'%0.4f') char(177) price.puterr])
        else
            disp(['The put price is $' num2str(price.putN(sample.N),'%0.4f') char(177) price.puterr])
        end
    else
        if notamer; 
            ...
            disp(['   the call price is $' num2str(price.callcv,'%0.4f') char(177) price.callcverr])
            disp(['   and the put price is $' num2str(price.putcv,'%0.4f') char(177) price.putcverr])
        else
            disp('Without control variates:')
            disp(['   the put price is $' num2str(price.putN(sample.N),'%0.4f') char(177) price.puterr])
            disp(['With control variates ' asset.control{:}])
            disp(['   the put price is $' num2str(price.putcv,'%0.4f') char(177) price.putcverr])
        end
    end
