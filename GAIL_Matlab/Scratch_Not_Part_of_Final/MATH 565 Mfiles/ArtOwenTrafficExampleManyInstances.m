...
for j=1:ninstance   
    ...
    avgvelocity(j)=sum(x(T+1,:)-x(T0+1,:))/(N*T1); %Average velocity of all cars
end
meanavgvel=mean(avgvelocity);
stdavgvel=std(avgvelocity);

%% Display results
display(['Time for simulation = ' num2str(toc(tstart)) ' seconds'])
display(['Average velocity of ' num2str(N) ' cars = ' num2str(meanavgvel) ...
    ' +/- ' num2str(2.58*stdavgvel/sqrt(ninstance))])
display(['     which is ' num2str(100*meanavgvel/vmax) ...
    '% of the ideal of ' num2str(vmax)])
display(['Flux = ' num2str(meanavgvel*N) ...
    ' +/- ' num2str(2.58*N*stdavgvel/sqrt(ninstance)) ...
    ' cars per unit time'])
