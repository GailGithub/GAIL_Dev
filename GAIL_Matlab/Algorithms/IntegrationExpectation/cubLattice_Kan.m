...
elseif FuncCount>1
    ycv = f(xpts);
    y = ycv(:,1); yval = y;
    yg1 = bsxfun(@minus,ycv(:,2:FuncCount),ycv(:,1));
    yg = [ycv(:,(FuncCount+1):end) yg1]; %yvalg = yg;
end
...
for m=out_param.mmin+1:out_param.mmax
    ...
    elseif FuncCount>1
        ycvnext = f(xnext);
        yg2=bsxfun(@minus,ycvnext(:,2:FuncCount),ycvnext(:,1));
        % yg3=[ycvnext(:,3:end) yg2];
        yg3 = [ycvnext(:,FuncCount+1:end) yg2];
        
        ynext = ycvnext(:,1) - yg3 *beta;
        yval=[yval; ynext];
    end
    ...
    if out_param.betaUpdate==0
        ...
        Y=Y(:,1);
        w=2;
        Y2=bsxfun(@minus, Y(:,q:w), Y(:,1));
        X=[Y2 X]

        yval=ycv(:,1)-ycv(:,q:end)*beta;
        y=yval; 
