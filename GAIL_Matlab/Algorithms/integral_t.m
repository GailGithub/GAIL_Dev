...
%   of x.
%
%   q = INTEGRAL_T(f,in_param) computes q, the definite integral of
...
% 
%   q = INTEGRAL_T(f,a,b,abstol,nlo,nhi,nmax) computes q, the definite
...
%
%   q =
%   INTEGRAL_T(f,'a',a,'b',b,'abstol',abstol,'nlo',nlo,'nhi',nhi,,'nmax',nmax)
...
%
%   [q, out_param] = INTEGRAL_T(f,...) returns the approximated 
...
%
%   Example 2:
%   >> f = @(x) exp(-x.^2); q = integral_t(f,'a',1,'b',2,'nlo',100,'nhi',10000,'abstol',1e-5,'nmax',1e7)
%   q = 0.1353
%
%
%   Example 3:
...
[f,out_param, flip] = integral_t_param(varargin{:});
...
if intervallen
    ...
    while true
        ...
        if out_param.tau*(Gf/intervallen+Ff/(2*ntrap)) < Ff %f lies outside cone
            ...
            warning('MATLAB:integral_t:peaky','This integrand is peaky relative to ninit. You may wish to increase ninit for similar integrands.');
        ...
        if ntrap*inflation+1 > out_param.nmax
            ...
            warning('MATLAB:integral_t:exceedbudget','integral_t attempts to exceed the cost budget. The answer may be unreliable.');
...
function [f, out_param, flip] = integral_t_param(varargin)
% parse the input to the integral_t function
...
if (out_param.a == inf||out_param.a == -inf||isnan(out_param.a)==1)
    warning('MATLAB:integral_t:anoinfinity',['a can not be infinity nor NaN. Use default a = ' num2str(default.a)])
    out_param.a = default.a;
end;
if (out_param.b == inf||out_param.b == -inf||isnan(out_param.b)==1)
    warning('MATLAB:integral_t:bnoinfinity',['b can not be infinity not Nan. Use default b = ' num2str(default.b)])
    out_param.b = default.b;
end;
...
if (out_param.abstol <= 0 )
    warning(['MATLAB:integral_t:abstolnonpos','Error tolerance should be greater than 0.' ...
...
if (~isposint(out_param.nlo))
    if isposge3(out_param.nlo)
        warning('MATLAB:integral_t:lowinitnotint',['Lowest initial number of points should be a positive integer.' ...
            ' Using ', num2str(ceil(out_param.nlo))])
        out_param.nlo = ceil(out_param.nlo);
    else
        warning('MATLAB:integral_t:lowinitlt3',['Lowest initial number of points should be a positive integer.' ...
...
if (~isposint(out_param.nhi))
    if isposge3(out_param.nhi)
        warning('MATLAB:integral_t:highinitnotint',['Highest initial number of points should be a positive integer.' ...
            ' Using ', num2str(ceil(out_param.nhi))])
        out_param.nhi = ceil(out_param.nhi);
    else
        warning('MATLAB:integral_t:highinitlt3',['Highest initial number of points should be a positive integer.' ...
...
if (out_param.nlo > out_param.nhi)
    if isposge3(out_param.nhi)
        warning('MATLAB:integral_t:nlobtnhi',['Highest initial number of points should be at least equal to to lowest initial number of points.' ...
...
if (~isposint(out_param.nmax))
    if ispositive(out_param.nmax)
        warning('MATLAB:integral_t:budgetnotint',['Cost budget should be a positive integer.' ...
            ' Using cost budget ', num2str(ceil(out_param.nmax))])
        out_param.nmax = ceil(out_param.nmax);
    else
        warning('MATLAB:integral_t:budgetisneg',['Cost budget should be a positive integer.' ...
