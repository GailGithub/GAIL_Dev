function [fmin,out_param]=funmin_g_CSC(varargin)
%funmin_g_CSC 1-D guaranteed global minimum value on [a,b] and the subset
%containing optimal solutions
%
%   fmin = funmin_g_CSC(f) finds minimum value of function f on the default
...
%
%   fmin = funmin_g_CSC(f,a,b,abstol,TolX) finds minimum value of
...
%
%   fmin = funmin_g_CSC(f,'a',a,'b',b,'abstol',abstol,'TolX',TolX)
...
%
%   fmin = funmin_g_CSC(f,in_param) finds minimum value of function f on the
...
%
%   [fmin, out_param] = funmin_g_CSC(f,...) returns minimum value fmin of
...
%  Example 1:
%
%  >> f=@(x) (x-0.3).^2+1; [fmin,out_param] = funmin_g_CSC(f)
%
%  fmin =
% 
%     1.0000
...
%  >> in_param.nmax = 10^6;
%  >> [fmin,out_param] = funmin_g_CSC(f,in_param)
% 
%  fmin =
% 
%     1.0000
...
%
%  >> f=@(x) (x-0.3).^2+1;
%  >> [fmin,out_param] = funmin_g_CSC(f,'a',-2,'b',2,'nhi',100,'nlo',10,'nmax',1e6,'abstol',1e-4,'TolX',1e-2)
%
%  fmin =
% 
%     1.0000
...
[f,out_param] = funmin_g_CSC_param(varargin{:});
...
if tauchange == 1
    warning('GAIL:funmin_g_CSC:peaky','This function is peaky relative to ninit. You may wish to increase ninit for similar functions.')
end;

% check cost budget flag
if out_param.exitflag == 1
    n = l;
    warning('GAIL:funmin_g_CSC:exceedbudget','funmin_g_CSC attempted to exceed the cost budget. The answer may be unreliable.')
end
...
function [f, out_param] = funmin_g_CSC_param(varargin)
% Parse the input to the funmin_g_CSC.m function
...
if isempty(varargin)
    warning('GAIL:funmin_g_CSC:nofunction','Function f must be specified. Now funmin_g_CSC will use f(x)=(x-0.3)^2+1.')
    help funmin_g_CSC
    f = @(x) (x-0.3).^2+1;
    out_param.f = f;
else
...
if (out_param.a == inf||out_param.a == -inf||isnan(out_param.a)==1)
    warning('GAIL:funmin_g_CSC:anoinfinity',['a cannot be infinity. Use default a = ' num2str(default.a)])
    out_param.a = default.a;
end;
if (out_param.b == inf||out_param.b == -inf||isnan(out_param.b)==1)
    warning('GAIL:funmin_g_CSC:bnoinfinity',['b cannot be infinity. Use default b = ' num2str(default.b)])
    out_param.b = default.b;
end;

% b is greater than a
if (out_param.b < out_param.a)
    warning('GAIL:funmin_g_CSC:blea','b cannot be smaller than a; exchange these two. ')
    tmp = out_param.b;
    out_param.b = out_param.a;
    out_param.a = tmp;
elseif(out_param.b == out_param.a)
    warning('GAIL:funmin_g_CSC:beqa',['b cannot equal to a. Use b = ' num2str(out_param.a+1)])
    out_param.b = out_param.a+1;
end;

% Check whether the error tolerance is nonnegative
if out_param.abstol < 0
    warning('GAIL:funmin_g_CSC:abstolnonpos', ['Error tolerance should be greater than or equal to 0.' ...
...
if out_param.TolX < 0
    warning('GAIL:funmin_g_CSC:Xtolnonpos', ['X tolerance should be greater than or equal to 0.' ...
...
if (~gail.isposint(out_param.nmax))
    if gail.ispositive(out_param.nmax)
        warning('GAIL:funmin_g_CSC:budgetnotint',['Cost budget should be a positive integer.' ...
            ' Using cost budget ', num2str(ceil(out_param.nmax))]);
        out_param.nmax = ceil(out_param.nmax);
    else
        warning('GAIL:funmin_g_CSC:budgetnotposint', ['Cost budget should be a positive integer.'...
        ' funmin_g_CSC will use the default budget' num2str(out_param.nmax)]);
        out_param.nmax = default.nmax;
    end
end
...
if (~gail.isposint(out_param.nlo))
    if gail.isposge3(out_param.nlo)
        warning('GAIL:funmin_g_CSC:lowinitnotint',['Lower bound of initial number of points should be a positive integer.' ...
            ' Using ', num2str(ceil(out_param.nlo)) ' as nlo '])
        out_param.nlo = ceil(out_param.nlo);
    else
        warning('GAIL:funmin_g_CSC:lowinitlt3',[' Lower bound of initial number of points should be a positive integer.' ...
...
if (~gail.isposint(out_param.nhi))
    if gail.isposge3(out_param.nhi)
        warning('GAIL:funmin_g_CSC:hiinitnotint',['Upper bound of initial number of points should be a positive integer.' ...
            ' Using ', num2str(ceil(out_param.nhi))])
        out_param.nhi = ceil(out_param.nhi);
    else
        warning('GAIL:funmin_g_CSC:hiinitlt3',[' Upper bound of points should be a positive integer.' ...
            ' Using default number of points ' int2str(default.nhi)])
        out_param.nhi = default.nhi;
    end
end
if (out_param.nlo > out_param.nhi)
    warning('GAIL:funmin_g_CSC:logrhi', 'Lower bound of initial number of points is larger than upper bound of initial number of points; Use nhi as nlo')
    out_param.nhi = out_param.nlo;
end;
if (out_param.nlo > out_param.nmax)
    warning('GAIL:funmin_g_CSC:logecost',['Lower bound of initial number of points should be smaller than cost budget.' ...
