...
%   specific generalized error tolerance,
%   tolfun:=max(abstol,reltol*|integral(f)|). The generalized tolerance
%   function can aslo be cosen as
%   tolfun:=theta*abstol+(1-theta)*reltol*|integral(f)| where theta is
%   another input parameter. Input f is a function handle. f should accept
%   an n x d matrix input, where d is the dimension of the hypercube, and n
%   is the number of points being evaluated simultaneously. The input d is
%   the dimension in which the function f is defined. Given the
