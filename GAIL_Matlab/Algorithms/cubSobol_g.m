...
%
% >> f = @(x) x(:,1).^2.*x(:,2).^2.*x(:,3).^2+0.11; hyperbox = [zeros(1,3);ones(1,3)]; q = cubSobol_g(f,hyperbox,'normal',1e-3,1e-3)
% q = 1.1***
% 
%
% Example 3: 
...
% sigma=0.05 and T=1.
% 
% >> f = @(x) exp(-0.05^2/2)*max(100*exp(0.05*x)-100,0); hyperbox = [zeros(1,1);ones(1,1)]; q = cubSobol_g(f,hyperbox,'normal',1e-4,1e-1,'fudge',@(m) 2.^-(2*m))
% q = 2.05***
%
%
% Example 5:
