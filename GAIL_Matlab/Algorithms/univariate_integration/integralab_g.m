...
while true
    ...
    Gf=sum(abs(df-(fpts(ntrap+1)-fpts(1))/ntrap))/intervallen; %approx weak norm
    Ff=ntrap*(sum(abs(diff(df))))/intervallen^2; %approx strong norm
    
    %Check necessary condition for integrand to lie in cone
    if out_param.tau*(Gf+Ff*intervallen/(2*ntrap)) < Ff*intervallen %f lies outside cone
        out_param.tau = 2*Ff*intervallen/(Gf+Ff*intervallen/(2*ntrap)); %increase tau
