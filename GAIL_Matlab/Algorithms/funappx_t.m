function [pp,out_param]=funappx_t(varargin)
%FUNAPPX_T 1-D guaranteed function recovery on a closed interval [a,b]
%
%   pp = FUNAPPX_T(f) approximates function f on the default interval [0,1]
...
%   
%   pp = FUNAPPX_T(f,a,b,abstol,nlo,nhi,nmax) for a given function f and
...
%   in different order.
%
%   pp = FUNAPPX_T(f,in_param) recovers function f on the finite interval
...
%
%   >> f = @(x) x.^2;
%   >> [pp, out_param] = funappx_t(f,-2,2,1e-7,10,10,1000000)
% 
% pp = 
% 
%       form: 'pp'
...
%
%   >> f = @(x) x.^2;
%   >> [pp, out_param] = funappx_t(f,'a',-2,'b',2,'nhi',100,'nlo',10)
%
% pp = 
% 
%       form: 'pp'
...
%   >> [pp, out_param] = funappx_t(f,in_param)
%
% pp = 
% 
%       form: 'pp'
...
[f, out_param] = funappx_t_param(varargin{:});
...
if out_param.exceedbudget == 1;
    ...
    warning('MATLAB:funappx_t:exceedbudget','funappx_t attempted to exceed the cost budget. The answer may be unreliable.')
end;

if tauchange == 1;
    warning('MATLAB:funappx_t:peaky','This function is peaky relative to nlo and nhi. You may wish to increase nlo and nhi for similar functions.')
end;
...
function [f, out_param] = funappx_t_param(varargin)
% parse the input to the funappx_t function
...
if isempty(varargin)
    warning('MATLAB:funappx_t:nofunction','Function f must be specified. Now GAIL is using f(x)=x^2 and unit interval [0,1].')
    help funappx_t
    f = @(x) x.^2;
    out_param.f = f;
else
...
if (out_param.a == inf||out_param.a == -inf||isnan(out_param.a)==1)
    warning('MATLAB:funappx_t:anoinfinity',['a can not be infinity. Use default a = ' num2str(default.a)])
    out_param.a = default.a;
end;
if (out_param.b == inf||out_param.b == -inf||isnan(out_param.b)==1)
    warning('MATLAB:funappx_t:bnoinfinity',['b can not be infinity. Use default b = ' num2str(default.b)])
    out_param.b = default.b;
end;

if (out_param.b < out_param.a)
    warning('MATLAB:funappx_t:blea','b can not be smaller than a; exchange these two. ')
    tmp = out_param.b;
    out_param.b = out_param.a;
    out_param.a = tmp;
elseif(out_param.b == out_param.a)
    warning('MATLAB:funappx_t:beqa',['b can not equal a. Use b = ' num2str(out_param.a+1)])
    out_param.b = out_param.a+1;
end;

% let error tolerance greater than 0
if (out_param.abstol <= 0 )
    warning(['MATLAB:funappx_t:abstolnonpos ','Error tolerance should be greater than 0.' ...
...
if (~isposint(out_param.nmax))
    if ispositive(out_param.nmax)
        warning('MATLAB:funappx_t:budgetnotint',['Cost budget should be a positive integer.' ...
            ' Using cost budget ', num2str(ceil(out_param.nmax))])
        out_param.nmax = ceil(out_param.nmax);
    else
        warning('MATLAB:funappx_t:budgetisneg',['Cost budget should be a positive integer.' ...
...
%     warning('MATLAB:funappx_t:logrhi', 'Lower bound of initial number of points is larger than upper bound of initial number of points; exchange these two')
%     temp = out_param.nlo;
...
if (~isposint(out_param.nlo))
    if isposge3(out_param.nlo)
        warning('MATLAB:funappx_t:lowinitnotint',['Lower bound of initial number of points should be a positive integer.' ...
            ' Using ', num2str(ceil(out_param.nlo))])
        out_param.nlo = ceil(out_param.nlo);
    else
        warning('MATLAB:funappx_t:lowinitlt3',[' Lower bound of initial number of points should be a positive integer.' ...
...
if (~isposint(out_param.nhi))
    if isposge3(out_param.nhi)
        warning('MATLAB:funappx_t:hiinitnotint',['Upper bound of initial number of points should be a positive integer.' ...
            ' Using ', num2str(ceil(out_param.nhi))])
        out_param.nhi = ceil(out_param.nhi);
    else
        warning('MATLAB:funappx_t:hiinitlt3',[' Upper bound of points should be a positive integer.' ...
            ' Using default number of points ' int2str(default.nhi)])
        out_param.nhi = default.nhi;
    end
end
if (out_param.nlo > out_param.nmax)
    warning('MATLAB:funappx_t:logecost',['Lower bound of initial number of points should be smaller than cost budget.' ...
            ' Using ', num2str(ceil(out_param.nmax/2))])
    out_param.nlo = out_param.nmax/2;
end;
if (out_param.nhi > out_param.nmax)
    warning('MATLAB:funappx_t:higecost',['Upper bound of initial number of points should be smaller than cost budget.' ...
