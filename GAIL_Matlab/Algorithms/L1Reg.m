...
% modified by TatsuLee : Mar-27-2009 

function x = L1Reg(A,b,abstol)
...
xOld    = x;
xOld(1) = xOld(1) + 1e-3;  % Force divergence

% Repeat until convergence
while (max(abs(x - xOld)) > 1e-4)
    % Move old coefficients
    xOld = x;
    
    % Calculate new observation weights (based on residuals from old coefficients)
    W = sqrt(1 ./ max(abs(( A * xOld) - b), 1e-4));  % Floor to avoid division by zero
