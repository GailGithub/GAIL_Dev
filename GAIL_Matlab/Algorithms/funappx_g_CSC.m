function [fappx,out_param]=funappx_g_CSC(varargin)
%funappx_g_CSC 1-D guaranteed locally adaptive function approximation (or
%   function recovery) on [a,b]
%
%   fappx = funappx_g_CSC(f) approximates function f on the default interval
...
%
%   fappx = funappx_g_CSC(f,a,b,abstol) for a given function f and the ordered
%   input parameters that define the finite interval [a,b], and a
%   guaranteed absolute error tolerance abstol.
%
%   fappx = funappx_g_CSC(f,'a',a,'b',b,'abstol',abstol) approximates function
...
%
%   fappx = funappx_g_CSC(f,in_param) approximates function f on the finite
...
%   value is used.
%
%   [fappx, out_param] = funappx_g_CSC(f,...) returns an approximated function
...
%
%   >> f = @(x) x.^2;
%   >> [~, out_param] = funappx_g_CSC(f,-2,2,1e-7,10,20)
%
%  out_param =
% 
%                a: -2
...
%
%   >> f = @(x) x.^2;
%   >> [~, out_param] = funappx_g_CSC(f,'a',-2,'b',2,'nhi',20,'nlo',10)
%
% out_param = 
% 
%                a: -2
...
%   >> [~, out_param] = funappx_g_CSC(f,in_param)
%
% out_param = 
% 
%                a: -5
...
while(max(err) > abstol)
    ...
    if(length(x) + counterr *(ninit -1) > out_param.nmax)
        out_param.exit(1) = 1;
        warning('GAIL:funappx_g_CSC:exceedbudget',['funappx_g_CSC attempted to',...
    ...
    if(iter==out_param.maxiter)
        out_param.exit(2) = 1;
        warning('GAIL:funappx_g_CSC:exceediter',['Number of iterations has'...
...
% parse the input to the funappx_g_CSC function
...
if isempty(varargin)
  warning('GAIL:funappx_g_CSC:nofunction',['Function f must be specified. '...
  ...
  if gail.isfcn(varargin{1})
    ...
    warning('GAIL:funappx_g_CSC:notfunction',['Function f must be a '...
...
if (out_param.a == inf||out_param.a == -inf)
    warning('GAIL:funappx_g_CSC:aisinf',['a cannot be infinity.'...
        'Use default a = ' num2str(default.a)])
    out_param.a = default.a;
end;
if (out_param.b == inf||out_param.b == -inf)
    warning(['GAIL:funappx_g_CSC:bisinf','b cannot be infinity. '...
        'Use default b = ' num2str(default.b)])
    out_param.b = default.b;
end;

if (out_param.b < out_param.a)
    warning('GAIL:funappx_g_CSC:blea',['b cannot be smaller than a;'...
        ' exchange these two. '])
    tmp = out_param.b;
    out_param.b = out_param.a;
    out_param.a = tmp;
elseif(out_param.b == out_param.a)
    warning('GAIL:funappx_g_CSC:beqa',['b cannot equal a.'...
...
if (out_param.abstol <= 0 )
    warning('GAIL:funappx_g_CSC:tolneg', ['Error tolerance should be greater'...
...
if (~gail.isposint(out_param.nmax))
    if gail.isposintive(out_param.nmax)
        warning('GAIL:funappx_g_CSC:budgetnotint',['Cost budget should be '...
            'a positive integer. Using cost budget '...
            , num2str(ceil(out_param.nmax))])
        out_param.nmax = ceil(out_param.nmax);
    else
        warning('GAIL:funappx_g_CSC:budgetisneg',['Cost budget should be '...
...
if (out_param.nlo > out_param.nhi)
    warning('GAIL:funappx_g_CSC:logrhi', ['Lower bound of initial number of'...
...
if (~gail.isposint(out_param.maxiter))
    if gail.ispositive(out_param.maxiter)
        warning('GAIL:funappx_g_CSC:maxiternotint',['Max number of '...
            'iterations should be a positive integer. Using max number '...
            'of iterations as  ', num2str(ceil(out_param.maxiter))])
        out_param.nmax = ceil(out_param.nmax);
    else
        warning('GAIL:funappx_g_CSC:budgetisneg',['Max number of iterations'...
