function [tmu,out_param]=meanMCnew2_g(varargin)
% meanMCnew2_G Monte Carlo method to estimate the mean of a random variable.
%
%   tmu = meanMCnew2_G(Yrand) estimates the mean, mu, of a random variable Y to
...
%
%   tmu = meanMCnew2_G(Yrand,abstol,reltol,alpha,fudge,nSig,n1,tbudget,nbudget)
...
%
%   tmu = meanMCnew2_G(Yrand,'abstol',abstol,'reltol',reltol,'alpha',alpha,'fudge',fudge,'nSig',nSig,'n1',n1,'tbudget',tbudget,'nbudget',nbudget)
%   estimates the mean of a random variable Y to within a specified
...
%
%   [tmu, out_param] = meanMCnew2_G(Yrand,in_param) estimates the mean of a
...
% >> tmu=meanMCnew2_g(Yrand,in_param)
% tmu = 0.33***
%
%
% Example 3:
...
%
% >> tmu=meanMCnew2_g(@(n)exp(rand(n,1)),1e-3,0)
% tmu = 1.71***
%
%
% Example 4:
...
%
% >> tmu=meanMCnew2_g(@(n)cos(rand(n,1)),'reltol',1e-2,'abstol',0,'alpha',0.05)
% tmu = 0.84***
%
%
...
if isempty(varargin)
    help meanMCnew2_g
    warning('MATLAB:meanMCnew2_g:yrandnotgiven',...
...
if (~gail.isfcn(Yrand))
    warning('MATLAB:meanMCnew2_g:yrandnotfcn',...
...
if max(size(Yrand(5)))~=5 || min(size(Yrand(5)))~=1
    ...
    warning('MATLAB:meanMCnew2_g:yrandnotlengthN',...
...
if (out_param.abstol < 0)
    ...
    warning('MATLAB:meanMCnew2_g:abstolneg',...
...
if (out_param.reltol < 0 || out_param.reltol > 1)
    ...
    warning('MATLAB:meanMCnew2_g:reltolneg',...
...
if (out_param.alpha <= 0 ||out_param.alpha >= 1) 
    ...
    warning('MATLAB:meanMCnew2_g:alphanotin01',...
...
if (out_param.fudge <= 1) 
    ...
    warning('MATLAB:meanMCnew2_g:fudgelessthan1',...
...
if (~gail.isposge30(out_param.nSig)) 
    ...
    warning('MATLAB:meanMCnew2_g:nsignotposint',...
...
if (~gail.isposge30(out_param.n1)) 
    ...
    warning('MATLAB:meanMCnew2_g:n1notposint',...
...
if (out_param.tbudget < 0) 
    ...
    warning('MATLAB:meanMCnew2_g:timebudgetneg',...
        ['Time budget in seconds should be positive; '...
...
if (out_param.tbudget == 0) 
    %time budget in seconds should be positive
    warning('MATLAB:meanMCnew2_g:timebudget0',...
...
if (~gail.isposge30(out_param.nbudget)) 
    ...
    warning('MATLAB:meanMCnew2_g:nbudgetnotposint',...
...
% Handles errors in meanMCnew2_g and meanMC_g_param to give an exit with
%  information.
...
switch out_param.exit
    case 1 % not enough samples to estimate the mean.
        ...
        warning('MATLAB:meanMCnew2_g:maxreached',...
