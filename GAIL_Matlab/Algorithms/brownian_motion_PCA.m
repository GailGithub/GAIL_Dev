...
[Eigenvectors,Eigenvalues]=eig(Sigma,'vector'); %Retrieving the eigenvectors and eigenvalues of the matrix Sigma
[~,order] = sort(Eigenvalues,'descend');

A = Eigenvectors(:,order)*diag(Eigenvalues(order).^(1/2));  %Defining A, which is the matrix responsible for the trasformation of the random variables into the brownian sample
