...
% 
% >> f = @(x) x(:,1).^2.*x(:,2).^2.*x(:,3).^2+0.11; hyperbox = [-inf(1,3);inf(1,3)]; q = cubLattice_g(f,hyperbox,'normal',1e-3,1e-3,'transform','C1sin')
% q = 1.1***
% 
% 
% Example 3: 
...
% interval [-1,2)^2:
% 
% >> f = @(x) exp(-x(:,1).^2-x(:,2).^2); hyperbox = [-ones(1,2);2*ones(1,2)]; q = cubLattice_g(f,hyperbox,'uniform',1e-3,1e-2,'transform','C1')
% q = 2.6****
%
%
% Example 4: 
...
% sigma=0.05 and T=1.
% 
% >> f = @(x) exp(-0.05^2/2)*max(100*exp(0.05*x)-100,0); hyperbox = [-inf(1,1);inf(1,1)]; q = cubLattice_g(f,hyperbox,'normal',1e-4,1e-2,'fudge',@(m) 2.^-(2*m),'transform','C1sin')
% q = 2.0****
%
%
% Example 5:
...
% 
% >> f = @(x) 3./(5-4*(cos(2*pi*x)))+1e-3; hyperbox = [0;1]; q = cubLattice_g(f,hyperbox,'uniform',1e-5,0,'transform','id')
% q = 1.0****
%
%
