function [fappx,out_param]=funappxNoPenalty_g_yuhan(varargin)
...
%
%     out_param.n --- number of points we need to reach the
...
out_param = in_param; out_param = rmfield(out_param,'memorytest');
...
for iter_i = 1:out_param.maxiter,
    %% Stage 1: compute Bpm of each subinterval and approximate |f''(t)|
    len = diff(x);
    ...
    %h = [x(2)-a x(3)-a  x(4:n)-x(1:n-3)  b-x(n-2)  b-x(n-1)];
