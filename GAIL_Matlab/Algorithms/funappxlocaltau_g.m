function [pp,out_param]=funappxlocaltau_g(varargin)
%FUNAPPXLOCALTAU_G 1-D guaranteed function recovery on a closed interval [a,b]
%
%   pp = FUNAPPXLOCALTAU_G(f) approximates function f on the default interval [0,1]
...
%   
%   pp = FUNAPPXLOCALTAU_G(f,a,b,abstol,taulo,tauhi) for a given function f
...
%
%   pp =
%   FUNAPPXLOCALTAU_G(f,'a',a,'b',b,'abstol',abstol,'taulo',taulo,'tauhi',tauhi)
...
%   different order.
%
%   pp = FUNAPPXLOCALTAU_G(f,in_param) recovers function f on the finite
...
%
%   [pp, out_param] = FUNAPPXLOCALTAU_G(f,...) returns a piecewise polynomial
...
%
%
%   >> f = @(x) x.^2; [pp, out_param] = funappxlocaltau_g(f)
%
% pp = 
% 
%       form: 'pp'
...
%
%   >> f = @(x) x.^2;
%   >> [pp, out_param] = funappxlocaltau_g(f,-2,2,1e-7,10,10)
% 
% pp = 
% 
%       form: 'pp'
...
%
%   >> f = @(x) x.^2;
%   >> [pp, out_param] = funappxlocaltau_g(f,'a',-2,'b',2,'tauhi',100,'taulo',10)
%
% pp = 
% 
%       form: 'pp'
...
%   >> [pp, out_param] = funappxlocaltau_g(f,in_param)
%
% pp = 
%       form: 'pp'
...
tau = max(ceil(out_param.tauhi*(out_param.taulo/out_param.tauhi)^(1/(1+len))),2);
n = ceil((tau+1)/2)+1;
index = [1 n];
err = inf;
...
while(max(err) >= out_param.abstol)
    ...
    gn(tmp) = (n-1)/len*max(abs(diff_y-(y(b)-y(a))/(n-1)));
     %approximate the stronger norm of input function
    fn(tmp) = (n-1)^2/len^2*max(abs(diff(diff_y)));
    
    % Stage 3: satisfy necessary condition
    if tau(tmp)*(gn(tmp)+fn(tmp)*len/2/(n-1)) >= fn(tmp)*len;
        % Stage 4: check for convergence
        err(tmp) = tau(tmp)*len*gn(tmp)/(4*(n-1)*(2*n-2-tau(tmp)));
