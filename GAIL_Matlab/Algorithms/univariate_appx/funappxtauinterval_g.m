function [fappx,out_param]=funappxtauinterval_g(varargin)
...
n = ceil((out_param.tau+1)*(out_param.b-out_param.a)/2)+1;
% cost budget flag
...
while n < out_param.nmax;
    ...
    x = out_param.a:(out_param.b-out_param.a)/(n-1):out_param.b;
    y = f(x);
    diff_y = diff(y);
    ...
    if out_param.tau*(gn+fn*(out_param.b-out_param.a)/(2*n-2))>= fn;
        % Stage 3: check for convergence
        errbound = 4*out_param.abstol*(n-1)*(2*n-2-out_param.tau*...
            (out_param.b-out_param.a))/out_param.tau/...
    ...
    % Stage2: do not satisfy necessary condition
        ...
        % increase tau
        out_param.tau = 2*fn/(gn+fn*(out_param.b-out_param.a)/(2*n-2));
...
taumin = 2/h;
tauinimax = 50;
out_param.tau = taumin^(1/(1+h))*tauinimax^(h/(1+h));
