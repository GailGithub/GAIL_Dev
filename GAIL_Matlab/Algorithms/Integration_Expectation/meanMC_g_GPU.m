function [tmu,out_param]=meanMC_g_gpu(varargin)
% meanMC_g_gpu Monte Carlo method to estimate the mean of a random variable.
%
%   tmu = meanMC_g_gpu(Yrand) estimates the mean, mu, of a random variable Y to
...
%
%   tmu = meanMC_g_gpu(Yrand,abstol,reltol,alpha,fudge,nSig,n1,tbudget,nbudget)
...
%
%   tmu = meanMC_g_gpu(Yrand,'abstol',abstol,'reltol',reltol,'alpha',alpha,
...
%
%   [tmu, out_param] = meanMC_g_gpu(Yrand,in_param) estimates the mean of a
...
% >> in_param.alpha = 0.05; Yrand=@(n) gpuArray.rand(n,1).^2;
% >> tmu=meanMC_g_gpu(Yrand,in_param)
% tmu = 0.33***
%
%
% Example 3:
...
%
% >> tmu=meanMC_g_gpu(@(n)exp(gpuArray.rand(n,1)),1e-3,0)
% tmu = 1.71***
%
%
% Example 4:
...
%
% >> tmu=meanMC_g_gpu(@(n)cos(gpuArray.rand(n,1)),'reltol',1e-2,'abstol',0,'alpha',0.05)
% tmu = 0.84***
%
%
...
if out_param.reltol ==0
    ...
    if sig0up == 0; % if the variance is zero, just take n_sigma samples
        ...
        if out_param.n > out_param.nremain;
            ...
            meanMC_g_gpu_err(out_param); % print warning message
...
function  [Yrand,out_param] = meanMC_g_gpu_param(varargin)
...
if isempty(varargin)
    help meanMC_g_gpu
    warning('GAIL:meanMC_g_gpu:yrandnotgiven',...
        'Yrand must be specified. Now GAIL is using Yrand =@(n) gpuArray.rand(n,1).^2.')
    Yrand = @(n) gpuArray.rand(n,1).^2;
...
if (out_param.reltol < 0 || out_param.reltol > 1)
    ...
    warning('GAIL:meanMC_g_gpu:reltolneg',...
...
if (out_param.alpha <= 0 ||out_param.alpha >= 1)
    ...
    warning('GAIL:meanMC_g_gpu:alphanotin01',...
...
if (out_param.fudge <= 1)
    ...
    warning('GAIL:meanMC_g_gpu:fudgelessthan1',...
...
if (~gail.isposge30(out_param.nSig))
    ...
    warning('GAIL:meanMC_g_gpu:nsignotposint',...
...
if (~gail.isposge30(out_param.n1))
    ...
    warning('GAIL:meanMC_g_gpu:n1notposint',...
...
if (out_param.tbudget < 0)
    ...
    warning('GAIL:meanMC_g_gpu:timebudgetneg',...
        ['Time budget in seconds should be positive; '...
...
if (out_param.tbudget == 0)
    %time budget in seconds should be positive
    warning('GAIL:meanMC_g_gpu:timebudget0',...
...
if (~gail.isposge30(out_param.nbudget))
    ...
    warning('GAIL:meanMC_g_gpu:nbudgetnotposint',...
...
function out_param = meanMC_g_gpu_err(out_param)
% Handles errors in meanMC_g_gpu and meanMC_g_gpu_param to give an exit with
%  information.
...
switch out_param.exit
    case 1 % not enough samples to estimate the mean.
        ...
        warning('GAIL:meanMC_g_gpu:maxreached',...
