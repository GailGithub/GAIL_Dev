...
for i = 1:nrep;
    tic;
    [~, out_param] = funappx_g(f1,a,b,1e-6,9,100);
    t=toc;
    time(1,1) = time(1,1) + t;
    npoints(1,1) = out_param.npoints;
    tic;
    [~, out_param] = funappxlocal_g(f1,a,b,1e-6,9,100);
    t=toc;
    time(1,2) = time(1,2) + t;
    npoints(1,2) = out_param.npoints;
    tic;
    [~, out_param] = funappx_g(f2,a,b,1e-6,9,100);
    t=toc;
    time(2,1) = time(2,1) + t;
    npoints(2,1) = out_param.npoints;
    tic;
    [~, out_param] = funappxlocal_g(f2,a,b,1e-6,9,100);
    t=toc;
    time(2,2) = time(2,2) + t;
    npoints(2,2) = out_param.npoints;
    f3 = @(x) exp(-1000*(x-randa(i)).^2);
    tic;
    [~, out_param] = funappx_g(f3,a,b,1e-6,9,100);
    t=toc;
    time(3,1) = time(3,1) + t;
    npoints(3,1) = out_param.npoints;
    tic;
    [~, out_param] = funappxlocal_g(f3,a,b,1e-6,9,100);
    t=toc;
    time(3,2) = time(3,2) + t;
    ...
    tic;
    [~, out_param] = funappx_g(f4,a,b,1e-6,9,100);
    t=toc;
    time(4,1) = time(4,1) + t;
    npoints(4,1) = out_param.npoints;
    tic;
    [~, out_param] = funappxlocal_g(f4,a,b,1e-6,9,100);
    t=toc;
    time(4,2) = time(4,2) + t;
    npoints(4,2) = out_param.npoints;
    tic;
    [~, out_param] = funappx_g(f5,a,b,1e-6,9,100);
    t=toc;
    time(5,1) = time(5,1) + t;
    npoints(5,1) = out_param.npoints;
    tic;
    [~, out_param] = funappxlocal_g(f5,a,b,1e-6,9,100);
    t=toc;
    time(5,2) = time(5,2) + t;
