...
if [ $# -eq 0 ]
  then
    echo "mail_w: No arguments supplied"
    matlabVer=
else
  echo 'mail_w: Input arg' $1
  matlabVer=$1
fi
...
SUBJECT="Daily test results were WRONG:$matlabVer REPORT ${DATE_TODAY}"
echo "Please find the attached report" | mail -s "$SUBJECT" -a $fileattach -a "Test_Results$matlabVer.txt" ${TO_EMAIL_IDS}
...
# merged into one email echo "${begin}${nl}" | mail -s "$SUBJECT" -a "Test_Results_$matlabVer.txt" ${TO_EMAIL_IDS}
