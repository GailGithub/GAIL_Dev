%UT_FUNAPPXLOCAL_G unit test for funappxlocal_g_CSC
classdef ut_funappxlocal_g_CSC < matlab.unittest.TestCase
  ...
  methods(Test)
    ...
    function funappx_gOfx(testCase)
      ...
      in_param.nlo = 10;
      in_param.nhi = 100;
      [pp, ~] = funappxlocal_g_CSC(f,in_param);
      x = rand(10000,1);
    ...
    function funappx_gOfxab(testCase)
      ...
      [pp,result] = funappxlocal_g_CSC(f,in_param);
    ...
    function funappx_gOfblea(testCase)
      ...
      in_param.a = 2; 
      in_param.b = 1;  
      [pp, result] = testCase.verifyWarning(@()funappxlocal_g_CSC(f,in_param),'MATLAB:funappx_g:blea');
      x = rand(10000,1)*(result.b-result.a)+result.a;
      actualerr = max(abs(ppval(pp,x)-f(x)));
    ...
    function funappx_gOfnofunction(testCase)
      [pp, result] = testCase.verifyWarning(@()funappxlocal_g_CSC,'MATLAB:funappx_g:nofunction');
      x = rand(10000,1)*(result.b-result.a)+result.a;
    ...
    function funappx_gOfbeqa(testCase)
      ...
      in_param.a = 1; 
      in_param.b = 1;  
      [pp, result] = testCase.verifyWarning(@()funappxlocal_g_CSC(f,in_param),'MATLAB:funappx_g:beqa');
      x = rand(10000,1)*(result.b-result.a)+result.a;
      actualerr = max(abs(ppval(pp,x)-f(x)));
      testCase.verifyLessThanOrEqual(actualerr,result.abstol);
    end
