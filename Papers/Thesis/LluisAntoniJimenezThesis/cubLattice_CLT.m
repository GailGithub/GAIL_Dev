function [q,out_param,y,kappanumap] = cubLattice_CLT(varargin)
...
lattice_p = gail.lattice_gen(1,n0,out_param.d);
shifts = rand(sample_size, out_param.d);
y = [];
for i = 1:sample_size
    y=[y; mean(f(mod(bsxfun(@plus, lattice_p, shifts(i,:)),1)))]; %grab Lattice points
end
...
for m=out_param.mmin+1:out_param.mmax
   ...
   mnext=m-1;
   nnext=2^mnext;
   lattice_next = gail.lattice_gen(nnext+1,2*nnext,out_param.d);
    for i = 1:sample_size
        ynext=[ynext; mean(f(mod(bsxfun(@plus, lattice_next, shifts(i,:)),1)))]; %grab Lattice points
    end
   n0=n0+nnext;
   y = mean([y ynext], 2);
